...
def detect_intent(p):
    ...
    # Si la pregunta habla directamente de gasolina/combustible, la fuente
    # principal es la hoja CONSUMO COMBUSTIBLE. Solo una petición explícita de
    # presupuesto mantiene la consulta dentro de PRESUPUESTO TIARA 2026.
    fuel_terms = ("combustible", "gasolina", "diesel", "diésel", "consumo de combustible")
    explicit_budget = any(x in p for x in ("presupuesto", "presupuestado"))
    if any(x in p for x in fuel_terms) and not explicit_budget:
        return "combustible"
    ...
    if rubric and any(re.search(r"\b" + re.escape(x) + r"\b", p) for x in finance_words) and not operational_inventory:
    ...
    if any(x in p for x in (
        "ultimo cambio", "último cambio", "ultimo mantenimiento", "último mantenimiento",
        "cuando fue", "cuándo fue", "cuando se hizo", "cuándo se hizo",
        "cuando cambiaron", "cuándo cambiaron", "cuando se cambio", "cuándo se cambió",
        "ultima vez", "última vez", "que hicimos", "qué hicimos",
        "que se hizo", "qué se hizo", "realizamos", "realizado", "realizada",
    )): return "ultimo_mantenimiento"
    if any(x in p for x in (
        "que paso", "qué pasó", "que sucedio", "qué sucedió",
        "que problema tuvo", "qué problema tuvo", "que falla tuvo", "qué falla tuvo",
        "que le paso", "qué le pasó", "que problemas ha tenido", "qué problemas ha tenido",
    )): return "bitacora"
...
# ============================================================
# ENRUTAMIENTO POR HOJA DEL EXCEL
# ============================================================
# La intención de la pregunta decide primero qué hoja es la fuente principal.
# No se consulta una hoja ajena solo porque contenga una palabra parecida.
# Las funciones especializadas siguen haciendo los cálculos que necesitan;
# este índice sirve además como búsqueda directa y respaldo cuando una
# estructura especializada no encuentra el registro solicitado.
SOURCE_SHEETS = {
    "presupuesto": ("PRESUPUESTO TIARA 2026",),
    "combustible": ("CONSUMO COMBUSTIBLE ",),
    "mantenimiento": ("VITACORA BARCO EN LINEA",),
    "programado": ("CHECKLIST MANT. PREVENTIVO",),
...
SOURCE_NOISE = {
    "presupuesto": {"presupuesto", "presupuestado", "gasto", "gastamos", "gastado", "gaste", "costo", "costos", "cuanto", "cuanto", "dame", "quiero", "saber"},
    "combustible": {"combustible", "gasolina", "diesel", "diésel", "consumo", "cuanto", "dame", "quiero", "saber"},
    "mantenimiento": {"mantenimiento", "mantenimientos", "bitacora", "bitácora", "evento", "problema", "falla", "fallas", "que", "qué", "paso", "pasó", "sucedio", "sucedió", "cuando", "cuándo", "fue", "hizo", "hicieron", "hecho", "hacer", "ultimo", "último", "historial", "completo", "veces", "cambios", "cantidad", "cuantas", "cuántas", "dame", "quiero", "saber"},
    "programado": {"mantenimiento", "mantenimientos", "cuando", "cuándo", "toca", "proximo", "próximo", "faltan", "horas", "dame", "quiero", "saber"},
    "inventario": {"inventario", "existencias", "stock", "producto", "productos", "que", "qué", "hay", "tenemos", "dame", "quiero", "saber"},
    "facturas": {"factura", "facturas", "invoice", "dame", "quiero", "saber", "muestra", "muéstrame"},
    "limpieza": {"limpieza", "limpiar", "aseo", "dame", "quiero", "saber", "muestra", "muéstrame"},
    "documentos": {"permiso", "permisos", "seguro", "seguros", "garantia", "garantía", "documento", "documentos", "vence", "vencimiento", "expira", "expiracion", "expiración", "dame", "quiero", "saber"},
}


def _source_sheet_rows(data, source):
    rows = []
    sheet_names = SOURCE_SHEETS.get(source, ())
    all_sheets = data.get("sheets", {}) or {}
    for expected in sheet_names:
        target = norm(expected).replace(" ", "")
        for actual, sheet_data in all_sheets.items():
            if norm(actual).replace(" ", "") == target or target in norm(actual).replace(" ", "") or norm(actual).replace(" ", "") in target:
                if isinstance(sheet_data, dict):
                    for idx, vals in enumerate(sheet_data.get("rows", []) or [], 1):
                        rows.append((idx, vals))
                break
    return rows


def _source_query_terms(question, source):
    p = norm(question)
    noise = SOURCE_NOISE.get(source, set())
    terms = []
    for token in re.findall(r"[a-z0-9]+", p):
        if len(token) < 3 or token in STOPWORDS or token in noise:
            continue
        if token not in terms:
            terms.append(token)

    # Las fechas del Excel se serializan como YYYY-MM-DD. Si el usuario
    # menciona un mes por nombre, añadimos su representación numérica para
    # poder encontrarlo directamente en la hoja.
    for month_name, month_number in MONTHS.items():
        if month_name in p:
            for marker in (f"-{month_number:02d}-", f"/{month_number:02d}/"):
...
def search_source_rows(data, question, source, limit=10, system=None):
    """Busca dentro de la hoja que corresponde al tema de la pregunta."""
    rows = _source_sheet_rows(data, source)
    if not rows:
        return []
    terms = _source_query_terms(question, source)
    scored = []
    for row_number, vals in rows:
        if not isinstance(vals, (list, tuple)):
            continue
        cell_texts = [norm(v) for v in vals if v not in (None, "")]
        row_text = " | ".join(cell_texts)
        if not row_text:
            continue

        # Cuando el sistema ya fue identificado, no mezclamos filas de otro
        # equipo aunque compartan palabras como "mantenimiento" o "cambio".
        if source == "mantenimiento" and system in SYSTEM_ALIASES:
            aliases = SYSTEM_ALIASES[system]
            if not any(re.search(r"\b" + re.escape(norm(alias)) + r"\b", row_text) for alias in aliases):
                continue
        ...
        if score:
            row_date = None
            for value in vals:
                row_date = parse_date(value) or excel_serial_to_date(value)
                if row_date:
                    break
            scored.append((score, row_date or date.min, row_number, vals))
    # La relevancia manda; cuando varias filas coinciden, la más reciente del
    # Excel aparece primero. Esto es especialmente importante en la Bitácora,
    # donde los eventos nuevos pueden estar antes/después de históricos.
    scored.sort(key=lambda x: (x[0], x[1], x[2]), reverse=True)
    return scored[:limit]


def answer_source_search(data, question, source, title=None, limit=10, system=None):
    """Respuesta genérica cuando la pregunta debe resolverse directamente desde una hoja."""
    hits = search_source_rows(data, question, source, limit=limit, system=system)
    sheet = SOURCE_LABELS.get(source, source)
    if not hits:
        return f"No encontré información relacionada en la hoja **{sheet}**."
    title = title or sheet
    lines = [f"### {title}", f"**Fuente:** {sheet}"]
    for score, row_date, row_number, vals in hits:
        text = " | ".join(str(v) for v in vals if v not in (None, ""))
        if text:
            lines.append(f"- **Fila {row_number}:** {text}")
    return "\n".join(lines)


def answer_cleaning(data, question):
    """Consulta la sección mensual de limpieza cuando se indica un mes."""
    p = norm(question)
    month = find_month(p)
    if not month:
        return answer_source_search(data, question, "limpieza", title="Limpieza y otros", limit=12)

    rows = _source_sheet_rows(data, "limpieza")
    if not rows:
        return "No hay información disponible en la hoja **S.LIMPIEZA & OTROS**."

    # En esta hoja cada bloque mensual comienza con una fecha aislada.
    starts = []
    for row_number, vals in rows:
        for value in vals:
            d = parse_date(value) or excel_serial_to_date(value)
            if d and d.month == month:
                starts.append((row_number, d))
                break
    if not starts:
        return answer_source_search(data, question, "limpieza", title="Limpieza y otros", limit=12)

    # Preferir el año pedido; si no se indicó, usar 2026.
    year = find_year(p) or 2026
    starts_year = [x for x in starts if x[1].year == year]
    if starts_year:
        start_row = min(x[0] for x in starts_year)
    else:
        start_row = min(x[0] for x in starts)
    ...
    for row_number, vals in rows:
        for value in vals:
            d = parse_date(value) or excel_serial_to_date(value)
            if d:
                all_date_rows.append(row_number)
                break
    future_starts = [r for r in all_date_rows if r > start_row]
    if future_starts:
        end_row = min(future_starts)
    ...
    for row_number, vals in rows:
        ...
        if text:
            text_n = norm(text)
            if not text_n.startswith("envase | productos de limpieza") and not text_n.startswith("total"):
                selected.append((row_number, text))
        if len(selected) >= 35:
            break

    if not selected:
        return f"No encontré registros de limpieza para {MONTH_NAMES[month]} {year}."
    lines = [
        f"### Limpieza y otros — {MONTH_NAMES[month].capitalize()} {year}",
        "**Fuente:** S.LIMPIEZA & OTROS",
    ]
    for row_number, text in selected:
        lines.append(f"- **Fila {row_number}:** {text}")
    return "\n".join(lines)


def maintenance_query_is_programmed(p):
    p = norm(p)
    return any(x in p for x in (
        "cuando toca", "cuándo toca", "proximo", "próximo", "faltan horas",
        "que mantenimiento toca", "qué mantenimiento toca",
        "programado", "programada", "pendiente de mantenimiento",
    ))
...
def _maintenance_log_text(rec):
    d = rec.get("data") or {}
    return norm(" ".join(
        str(d.get(k) or "")
        for k in ("observacion", "observaciones", "sistema", "estado")
    ))


def _maintenance_log_candidates(data, thought):
    """Selecciona SOLO eventos de mantenimiento de la Bitácora.

    La Bitácora es la fuente de verdad para trabajos realmente realizados.
    El Checklist se reserva para mantenimiento programado/proximo.
    """
    ...
    # Un mantenimiento real debe describir una acción de servicio/cambio/
    # revisión/reparación. No basta con que la fila mencione "motor".
    action_terms = (
        "mantenimiento", "cambio", "cambiaron", "cambiado", "servicio",
        "revision", "revisión", "reemplazo", "remplazo", "reparacion",
        "reparación", "instalacion", "instalación", "instalado", "ajuste",
        "ajustar", "limpieza",
    )

    component_terms = []
    if "aceite de pata" in p:
        component_terms = ["aceite de pata"]
    elif "aceite" in p:
        component_terms = ["aceite"]
    elif "filtro racor" in p or "racor" in p:
        component_terms = ["racor"]
    elif "filtro" in p:
        component_terms = ["filtro"]
    elif "candela" in p or "bujia" in p or "bujía" in p:
        component_terms = ["candela", "bujia", "bujía"]
    elif "impeler" in p or "impeller" in p or "empeler" in p:
        component_terms = ["impeler", "impeller", "empeler"]
    elif "anodo" in p or "ánodo" in p:
        component_terms = ["anodo", "ánodo"]
    elif "kit 300" in p or "300 horas" in p:
        component_terms = ["kit 300", "300 horas"]

    candidates = []
    for rec in rows:
        d = rec.get("data") or {}
        text = _maintenance_log_text(rec)

        # Algunas filas combinan dos sistemas (por ejemplo, motores + patas).
        # En esos casos no descartamos el registro si el propio texto nombra
        # explícitamente el sistema consultado.
        system_match = d.get("sistema") == system
        if not system_match:
            if system == "Motores Mercury":
                system_match = bool(re.search(r"\b(?:motor|motores|mercury)\b", text))
            elif system == "Patas Mercury":
                system_match = bool(re.search(r"\b(?:pata|patas|muelas|outdrive)\b", text))
            elif system == "Generador Cummins Onan":
                system_match = bool(re.search(r"\b(?:generador|cummins|onan)\b", text))
        if not system_match:
            continue

        # Para una consulta genérica de mantenimiento, exigir que la propia
        # entrada describa un trabajo realizado, no un problema o una prueba.
        has_action = any(
        ...
        # "después del mantenimiento" describe el contexto de una falla,
        # no un trabajo de mantenimiento realizado en esa fila.
        if "despues del mantenimiento" in text and not re.search(
            r"\b(?:cambio|reemplazo|remplazo|reparacion|instalacion|instalado)\b",
            text,
        ):
            continue

        if component_terms and not any(re.search(r"\b" + re.escape(term) + r"\b", text) for term in component_terms):
            continue

        candidates.append(rec)

    candidates.sort(
        key=lambda r: (_log_date(r) or date.min, int(r.get("source_row") or 0)),
        reverse=True,
    )
    return candidates


def _format_single_maintenance_event(rec, title="Último mantenimiento", thought=None):
    d = rec.get("data") or {}
    report = _log_date(rec)
    install = _log_date(rec, "fecha_instalacion")
    text = str(d.get("observacion") or "").strip()
    ...
    # Si la fila contiene varios trabajos, mostrar solo la parte que responde
    # al componente solicitado cuando este puede identificarse con claridad.
    p = norm(thought.get("normalized", "")) if isinstance(thought, dict) else ""
    component_terms = []
    if "aceite de pata" in p:
        component_terms = ["aceite pata"]
    elif "aceite" in p:
        component_terms = ["aceite motor", "aceite motores"]
    elif "filtro racor" in p or "racor" in p:
        component_terms = ["racor"]
    elif "filtro" in p:
        component_terms = ["filtro"]
    if component_terms and "," in text:
        parts = [x.strip() for x in re.split(r"[,;]+", text) if x.strip()]
        selected = [x for x in parts if any(term in norm(x) for term in component_terms)]
        if selected:
            text = "; ".join(selected)

    lines = [f"### {title}"]
    if text:
        lines.append(f"**Trabajo:** {text}")
    if report:
        lines.append(f"**Fecha del reporte:** {fmt_date(report)}")
    if install and install != report:
        lines.append(f"**Fecha de instalación/atención:** {fmt_date(install)}")
    if d.get("horas") is not None:
        lines.append(f"**Horómetro:** {d['horas']:g} h")
    if notes:
        lines.append(f"**Observaciones:** {notes}")
    lines.append("**Fuente:** VITACORA BARCO EN LINEA")
    return "\n".join(lines)
...
def answer_ultimo_mantenimiento(data, thought):
    ...
    # "Último mantenimiento" significa el último trabajo REALIZADO.
    # La fuente es la Bitácora; el Checklist se reserva para lo programado.
    candidates = _maintenance_log_candidates(data, thought)
    if not candidates:
        return "No encontré un mantenimiento realizado en la Bitácora para ese sistema."

    return _format_single_maintenance_event(candidates[0], "Último mantenimiento", thought)
...
def answer_historial(data, thought, complete=False):
    ...
    rows = _maintenance_log_candidates(data, thought)
    if not rows:
        return "No encontré mantenimientos realizados en la Bitácora para ese sistema."

    limit = 15 if complete else 8
    lines = [f"### Historial de mantenimiento — {system}"]
    for rec in rows[:limit]:
        d = rec.get("data") or {}
        report = _log_date(rec)
        install = _log_date(rec, "fecha_instalacion")
        text_value = str(d.get("observacion") or "").strip()
        line = f"- **{fmt_date(report)}** — {text_value}" if report else f"- {text_value}"
        if install and install != report:
            line += f" — atendido {fmt_date(install)}"
        if d.get("horas") is not None:
            line += f" — {d['horas']:g} h"
        lines.append(line)
...
def answer_count(data, thought):
    system = thought.get("system")
    if system not in SYSTEM_ALIASES:
        return "No pude determinar el sistema para contar mantenimientos."
    ...
    return (
        f"### Cantidad de mantenimientos — {system}\n"
        f"**Total de registros encontrados:** {len(rows)}\n"
        "**Fuente:** VITACORA BARCO EN LINEA"
    )
...
def answer_fuel(data, thought):
    """Consulta exclusivamente la hoja CONSUMO COMBUSTIBLE para combustible."""
    ...
    if not rows:
        return "No hay datos disponibles en la hoja CONSUMO COMBUSTIBLE."

    # Hoja real: A MES, B GASOLINA LITROS, C DIESEL LITROS, G TOTAL.
    totals = {m: 0.0 for m in range(1, 13)}
    liters_gas = {m: 0.0 for m in range(1, 13)}
    liters_diesel = {m: 0.0 for m in range(1, 13)}
    found_total = {m: False for m in range(1, 13)}
    found_liters_gas = {m: False for m in range(1, 13)}
    found_liters_diesel = {m: False for m in range(1, 13)}
    ...
    for rec in rows:
        ...
        gas = safe_float(vals[1]) if len(vals) > 1 else None
        diesel = safe_float(vals[2]) if len(vals) > 2 else None
        total = safe_float(vals[6]) if len(vals) > 6 else None

        if gas is not None:
            liters_gas[d.month] += gas
            found_liters_gas[d.month] = True
        if diesel is not None:
            liters_diesel[d.month] += diesel
            found_liters_diesel[d.month] = True
        if total is not None and total != 0:
            totals[d.month] += total
            found_total[d.month] = True

    asks_liters = "litros" in p or "galones" in p
    asks_gas = "gasolina" in p
    asks_diesel = "diesel" in p or "diésel" in p

    def month_result(m):
        if asks_liters and asks_gas:
            return liters_gas[m], "Gasolina", "litros", found_liters_gas[m]
        if asks_liters and asks_diesel:
            return liters_diesel[m], "Diésel", "litros", found_liters_diesel[m]
        return totals[m], "Combustible", "USD", found_total[m]

    if month:
        value, label, unit, found = month_result(month)
        if not found:
            return (
                f"### Combustible — {MONTH_NAMES[month].capitalize()} {year}\n"
                f"**{label}:** NO DETERMINADO\n"
                "**Fuente:** CONSUMO COMBUSTIBLE"
            )
        value_text = f"{value:,.2f} litros" if unit == "litros" else money(value)
        return (
            f"### Combustible — {MONTH_NAMES[month].capitalize()} {year}\n"
            f"**{label}:** {value_text}\n"
            "**Fuente:** CONSUMO COMBUSTIBLE\n"
    ...
    for m in range(1, 13):
        value, label, unit, found = month_result(m)
        if found:
            value_text = f"{value:,.2f} litros" if unit == "litros" else money(value)
            lines.append(f"- **{MONTH_NAMES[m].capitalize()}:** {value_text}")
    if len(lines) == 1:
        lines.append("- No hay consumos registrados para ese año.")
...
def answer_inventory(data, thought):
    ...
    if not hits:
        # Una pregunta general de inventario debe abrir la hoja INVENTARIO,
        # no tratar "¿qué tenemos?" como si fuera un nombre de producto.
        if (
            not tokens
            or "que tenemos en inventario" in p
            or "qué tenemos en inventario" in p
            or "que hay en inventario" in p
            or "qué hay en inventario" in p
        ):
            inventory_hits = search_source_rows(data, p, "inventario", limit=50)
            inventory_hits = [
                h for h in inventory_hits
                if norm((h[3][1] if len(h[3]) > 1 else "")) not in {"total", "producto"}
                and norm((h[3][0] if len(h[3]) > 0 else "")) != "cantidad"
            ]
            inventory_hits.sort(key=lambda h: h[2])
            if inventory_hits:
                lines = ["### Inventario", "**Fuente:** INVENTARIO"]
                for _, _, row_number, vals in inventory_hits[:15]:
                    text = " | ".join(str(v) for v in vals if v not in (None, ""))
                    if text:
                        lines.append(f"- **Fila {row_number}:** {text}")
                return "\n".join(lines)
            return answer_source_search(data, p, "inventario", title="Inventario", limit=15)
...
def answer_log(data, thought):
    ...
    if re.search(r"\bestribor\b", p):
        requested_location = "estribor"
    elif re.search(r"\bbabor\b", p):
        requested_location = "babor"

    for rec in rows:
        d = rec.get("data") or {}
        text_row = _maintenance_log_text(rec)
        if system:
            system_match = d.get("sistema") == system
            if not system_match and system == "Motores Mercury":
                system_match = bool(re.search(r"\b(?:motor|motores|mercury)\b", text_row))
            elif not system_match and system == "Patas Mercury":
                system_match = bool(re.search(r"\b(?:pata|patas|muelas|outdrive)\b", text_row))
            elif not system_match and system == "Generador Cummins Onan":
                system_match = bool(re.search(r"\b(?:generador|cummins|onan)\b", text_row))
            if not system_match:
                continue
        if requested_location and requested_location not in text_row:
            continue
...
def answer_sheet(data, sheet_name, title, thought=None):
    """Busca en una hoja concreta y devuelve solo filas relacionadas."""
    ...
    tokens = [
        t for t in re.findall(r"[a-z0-9]+", p)
    ...
    for row in rows:
        ...
        # Fecha de la propia fila, cuando existe. Sirve para filtrar meses/años.
        row_dates = []
        for v in vals:
            d = parse_date(v) or excel_serial_to_date(v)
            if d:
                row_dates.append(d)

        if month:
            if not any(d.year == year and d.month == month for d in row_dates):
                continue
        elif thought.get("year"):
            if row_dates and not any(d.year == year for d in row_dates):
                continue

        score = sum(1 for tok in tokens if tok in text_n)

        # Para facturas/limpieza una consulta genérica no debe devolver las
        # primeras filas arbitrarias del Excel: preferimos las más recientes.
        latest = max(row_dates) if row_dates else date.min
        if score:
            scored.append((score, latest, row))
        elif not tokens:
            dated.append((latest, row))

    if scored:
        scored.sort(key=lambda x: (x[0], x[1]), reverse=True)
        selected = [x[2] for x in scored[:10]]
    elif dated:
        dated.sort(key=lambda x: x[0], reverse=True)
        selected = [x[1] for x in dated[:10]]
    else:
        return f"No encontré información relacionada en la hoja **{sheet_name}**."
...
def answer_general(data, thought):
    # Si la intención quedó general pero el sistema sí está identificado,
    # todavía podemos enviar la consulta a la hoja correcta.
    system = thought.get("system")
    if system == "Combustible":
        return answer_fuel(data, thought)
    if system == "Inventario":
        return answer_inventory(data, thought)
    if system == "Facturas":
        return answer_source_search(data, thought.get("normalized", ""), "facturas", title="Facturas", limit=12)
    if system == "Limpieza":
        return answer_cleaning(data, thought.get("normalized", ""))
    if system == "Permisos y seguros":
        return answer_documents(data)
    if system == "Motores Mercury" or system == "Patas Mercury" or system == "Generador Cummins Onan":
        return answer_source_search(
            data, thought.get("normalized", ""), "mantenimiento",
            title="Bitácora — resultados relacionados", limit=10, system=system
        )
    return ("Puedo consultar las 8 hojas del Excel: mantenimiento, bitácora, inventario, combustible, presupuesto, facturas, limpieza y permisos/seguros.\n\n"
...
def run_agent(data, question, context=None):
    ...
    if intent == "ultimo_mantenimiento":
        # La pregunta por mantenimiento realizado se busca primero en la
        # Bitácora en Línea. El checklist queda como respaldo si la bitácora
        # no contiene una coincidencia.
        result = answer_source_search(
            data, question, "mantenimiento",
            title="Bitácora — mantenimiento registrado", limit=8, system=thought.get("system")
        )
        if result.startswith("No encontré información relacionada"):
            return answer_ultimo_mantenimiento(data, thought)
        return result
    if intent == "historial_completo":
        result = answer_source_search(
            data, question, "mantenimiento",
            title="Bitácora — historial de mantenimiento", limit=15, system=thought.get("system")
        )
        if result.startswith("No encontré información relacionada"):
    ...
    if intent == "cantidad_mantenimiento":
        result = answer_source_search(
            data, question, "mantenimiento",
            title="Bitácora — registros de mantenimiento", limit=15, system=thought.get("system")
        )
        if result.startswith("No encontré información relacionada"):
    ...
    if intent == "mantenimiento":
        if maintenance_query_is_programmed(p):
            return answer_mantenimiento(data, thought)
        return answer_source_search(
            data, question, "mantenimiento",
            title="Bitácora — mantenimiento registrado", limit=10, system=thought.get("system")
        )
    ...
    if intent == "fotografias":
        ...
        return answer_photos(data, question)
    if intent == "facturas":
        return answer_source_search(data, question, "facturas", title="Facturas", limit=12)
    if intent == "limpieza":
        return answer_cleaning(data, question)
    if intent == "fotografias":
        return answer_photos(data, question)
