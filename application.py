...
def _salidas_barco_record(data):
    """Localiza exclusivamente la fila RUBROS=SALIDAS BARCO."""
    for rec in _budget_rows_from_excel_values(data):
...
def _salidas_barco_value(data, year, month=None):
    ...
    Solo utiliza la fila SALIDAS BARCO y el TOTAL oficial asociado.
    Nunca interpreta ni suma OBSERVACIONES.
    """
    ...
    d = rec.get("data") or {}
    # El formato actual puede guardar TOTAL en la propia fila.
    for key, value in d.items():
        if norm(key).replace(" ", "") == f"total{year}":
            return _budget_number(value)

    # Compatibilidad: algunos snapshots conservan TOTAL en la fila siguiente.
    source_row = rec.get("source_row")
    try:
    ...
    if source_row is not None:
        for candidate in _budget_rows_from_excel_values(data):
            try:
                candidate_row = int(candidate.get("source_row") or 0)
            except Exception:
                candidate_row = 0
            if candidate_row != source_row + 1:
                continue
            cd = candidate.get("data") or {}
...
def _financial_analysis(data, thought):
    ...
    # autorizado es combustible ÷ salidas, expresado en USD por salida.
...
def run_agent(data, question, context=None):
    ...
    # Consulta directa de SALIDAS BARCO, sin interpretar OBSERVACIONES.
