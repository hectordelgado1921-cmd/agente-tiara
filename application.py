...
def detect_system(p):
    """Identifica sistemas; las ubicaciones se extraen por separado."""
    p = norm(p)
    ...
    if any(x in p for x in ("motor", "motores", "mercury")): return "Motores Mercury"
    if any(x in p for x in ("combustible", "gasolina", "diesel", "diésel")): return "Combustible"
    # Se conservan estos términos porque ya sirven para consultas directas
    # de inventario de productos. No se convierten en nuevos comandos.
...
def detect_location(p):
    """Extrae ubicación sin convertirla en un sistema independiente."""
    p = norm(p)
    if any(x in p for x in ("estribor", "motor de estribor", "motor estribor")): return "estribor"
    if any(x in p for x in ("babor", "motor de babor", "motor babor")): return "babor"
    if "proa" in p: return "proa"
    if "popa" in p: return "popa"
    return None


COMPONENT_ALIASES = {
    "aceite": ("aceite",),
    "filtro de aceite": ("filtro de aceite", "filtros de aceite", "filtro aceite"),
    "filtro": ("filtro", "filtros"),
    "filtro racor": ("filtro racor", "racor"),
    "aceite de pata": ("aceite de pata",),
    "anodos": ("anodo", "anodos", "ánodo", "ánodos"),
    "kit 300 horas": ("kit 300", "kit de 300", "300 horas", "servicio 300"),
    "bomba de agua": ("bomba de agua", "bomba agua"),
    "bujias": ("bujia", "bujias", "bujía", "bujías"),
    "termostato": ("termostato",),
    "fajas": ("faja", "fajas"),
    "impeler": ("impeler", "impeller", "empeler"),
    "baterias": ("bateria", "baterias", "batería", "baterías"),
}


def detect_component(p):
    """Devuelve solo componentes que tienen utilidad real en consultas."""
    p = norm(p)
    # Más específico primero para evitar que 'filtro' gane a 'filtro racor'.
    for canonical, aliases in sorted(COMPONENT_ALIASES.items(), key=lambda item: -max(map(len, item[1]))):
...
def detect_intent(p):
    ...
    if any(x in p for x in ("ultimo cambio", "último cambio", "ultimo mantenimiento", "último mantenimiento", "cuando fue", "cuándo fue", "cuando se hizo", "cuándo se hizo", "cuando cambiaron", "cuándo cambiaron", "ultima vez", "última vez")): return "ultimo_mantenimiento"
    if any(x in p for x in ("kit 300", "kit de 300", "300 horas", "servicio 300")): return "kit_300"
    # Eventos y fallas se consultan en bitácora. Esto se evalúa antes de
    # mantenimiento para evitar que palabras como "cambio" o componentes
    # manden una pregunta histórica al módulo equivocado.
    if any(x in p for x in ("que paso", "qué pasó", "que sucedio", "qué sucedió", "que problema tuvo", "qué problema tuvo", "que falla tuvo", "qué falla tuvo")):
        return "bitacora"
...
def think(question):
    ...
    return {
        ...
        "component": detect_component(p),
        "location": detect_location(p),
        "month": find_month(p),
        "year": find_year(p),
        "normalized": p,
    }
    ...
    if typed_question:
        ...
        if audio_signature != st.session_state.voice_audio_signature:
            ...
            if voice_question:
                ...
                # Nunca dejamos la interfaz sin respuesta si la transcripción falla.
                # El error se muestra en el mismo campo de respuesta.
