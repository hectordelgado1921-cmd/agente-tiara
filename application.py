...
# ============================================================
# DICCIONARIO AMPLIADO / MAPA SEMÁNTICO DEL EXCEL
# ============================================================
# No depende de que el usuario use exactamente el nombre de una hoja.
# Incluye vocabulario coloquial, técnico, abreviaturas, términos de los
# encabezados reales y palabras que aparecen en los registros del libro.
SHEET_VOCABULARY = {
    "CHECKLIST MANT. PREVENTIVO": {
        "system": "Motores Mercury",
        "aliases": [
            "mantenimiento", "mantenimientos", "servicio", "servicios", "preventivo",
            "preventiva", "revision", "revisión", "cambio", "cambios", "aceite",
            "filtro", "filtros", "filtro de aceite", "filtros de aceite",
            "filtro gasolina", "racor", "racores", "anodo", "anodos", "ánodo",
            "ánodos", "pata", "patas", "pierna", "kit", "kit 300", "300 horas",
            "bomba de agua", "bujias", "bujía", "termostato", "fajas", "impeler",
            "impeller", "historial de mantenimiento", "cada horas", "intervalo"
        ]
    },
    "VITACORA BARCO EN LINEA": {
        "system": "Bitácora",
        "aliases": [
            "bitacora", "bitácora", "vitacora", "vitácora", "reporte", "reportes",
            "reporte de barco", "evento", "eventos", "incidente", "incidentes",
            "problema", "problemas", "falla", "fallas", "averia", "avería",
            "daño", "danos", "daños", "alarma", "alarmas", "codigo", "código",
            "codigos", "códigos", "pendiente", "pendientes", "listo", "resuelto",
            "reparacion", "reparación", "reparado", "instalacion", "instalación",
            "observaciones", "fecha reporte", "fecha de reporte", "fecha instalación",
            "horas/mo/ge", "horas", "horometro", "horómetro", "arnes", "arnés",
            "direccion hidraulica", "dirección hidráulica", "cilindro", "calentamiento",
            "calento", "calentó", "no calienta", "estribor", "babor"
        ]
    },
    "INVENTARIO": {
        "system": "Inventario",
        "aliases": [
            "inventario", "existencias", "stock", "productos", "producto", "cantidad",
            "cuanto hay", "cuánto hay", "cuantos hay", "cuántos hay", "disponible",
            "disponibles", "faltante", "faltantes", "compra", "compras", "valor unitario",
            "valor u", "precio total", "descuento", "tax", "observaciones",
            "vasos", "tazas", "platos", "cucharas", "cubiertos", "policarbonato",
            "jet i", "termicos", "térmicos", "fender", "chaleco", "salvavidas",
            "equipo", "herramienta", "herramientas", "repuesto en inventario"
        ]
    },
    "CONSUMO COMBUSTIBLE ": {
        "system": "Combustible",
        "aliases": [
            "combustible", "consumo combustible", "consumo de combustible", "gasolina",
            "diesel", "diésel", "litros", "litro", "galones", "galon", "galón",
            "llenado", "tanque", "tanques", "carga de combustible", "cargar combustible",
            "factura de combustible", "tax combustible", "total combustible", "salida",
            "dia salida", "día salida", "fecha2", "mes combustible", "consumo",
            "rendimiento", "gasto de gasolina", "gasto de diesel"
        ]
    },
    "PRESUPUESTO TIARA 2026": {
        "system": "Finanzas",
        "aliases": [
            "presupuesto", "presupuestado", "presupuestar", "gasto", "gastos", "gastado",
            "gastamos", "gaste", "gasté", "costo", "costos", "importe", "monto",
            "dinero", "cuanto se gasto", "cuánto se gastó", "cuanto gastamos",
            "cuánto gastamos", "rubro", "rubros", "categoria", "categoría", "total",
            "enero", "febrero", "marzo", "abril", "mayo", "junio", "julio",
            "agosto", "septiembre", "setiembre", "octubre", "noviembre", "diciembre",
            "mantenimiento anual", "mant.motores", "mant.generador", "repuestos",
            "cotizaciones", "seguros", "membresias", "membresía", "salario", "sueldo",
            "s.limpieza", "limpieza", "inventario.cocina", "cocina", "agua dulce",
            "hielo", "visitas", "juan manuel", "travel", "slip", "marina", "comida",
            "ccss", "otros gastos"
        ]
    },
    "FACTURAS 2026...": {
        "system": "Facturas",
        "aliases": [
            "factura", "facturas", "invoice", "invoices", "recibo", "recibos",
            "comprobante", "comprobantes", "compra", "proveedor", "proveedores",
            "pago", "pagos", "subtotal", "impuesto", "iva", "tax", "total factura",
            "numero factura", "número factura", "fecha factura", "facturado", "documento fiscal"
        ]
    },
    "S.LIMPIEZA & OTROS": {
        "system": "Limpieza",
        "aliases": [
            "limpieza", "limpiar", "aseo", "higiene", "productos de limpieza",
            "producto de limpieza", "detergente", "jabón", "jabon", "cloro", "desinfectante",
            "desengrasante", "limpiador", "paño", "pano", "esponja", "cepillo",
            "toalla", "papel", "basura", "bote", "cantidad bote", "stock limpieza",
            "compra limpieza", "valor u limpieza", "precio limpieza", "descuento limpieza",
            "tax limpieza", "gasto de limpieza"
        ]
    },
    "PERMISOS & SEGUROS": {
        "system": "Permisos y seguros",
        "aliases": [
            "permiso", "permisos", "navegacion", "navegación", "permiso de navegación",
            "miami", "costa rica", "seguro", "seguros", "ins", "aseguradora",
            "poliza", "póliza", "garantia", "garantía", "extension garantia",
            "extensión garantía", "seakeeper", "see keeper", "equipo salvamento",
            "salvamento", "seguridad", "vence", "vencimiento", "vencen", "expira",
            "expiracion", "expiración", "fecha de vencimiento", "vigencia", "renovar",
            "renovacion", "renovación", "registro", "numero de registro", "número de registro",
            "capitania", "capitanía", "documentacion", "documentación", "documento"
        ]
    },
}

# Rubros exactos del presupuesto + lenguaje cotidiano. Se usa como diccionario
# independiente para que una pregunta financiera pueda identificar el rubro
# aunque no diga literalmente "PRESUPUESTO".
RUBRIC_VOCABULARY = {
    "MANTENIMIENTO ANUAL TIARA": ["mantenimiento anual", "mant anual", "servicio anual", "mantenimiento general"],
    "MANT.MOTORES": ["mantenimiento motores", "mantenimiento de motores", "servicio motores", "motor mercury", "motores mercury", "filtros de motor", "aceite motores"],
    "MANT.GENERADOR": ["mantenimiento generador", "mantenimiento del generador", "servicio generador", "aceite generador", "filtro generador"],
    "COMSUMO COMBUSTIBLE": ["combustible", "gasolina", "diesel", "diésel", "consumo", "combustible barco", "gasolina barco"],
    "REPUESTOS & COTIZACIONES": ["repuestos", "repuesto", "cotizaciones", "cotizacion", "pieza", "piezas", "partes", "recambio", "refaccion", "refacción"],
    "SEGUROS & MEMBRESIAS": ["seguros", "seguro", "membresias", "membresía", "membresías", "poliza", "póliza", "renovacion seguro", "renovación seguro"],
    "SALARIO ANDRES": ["salario", "sueldo", "pago salario", "mi salario", "pago de andres", "pago andres"],
    "S.LIMPIEZA": ["limpieza", "aseo", "productos de limpieza", "gasto limpieza", "compra limpieza"],
    "INVENTARIO.COCINA": ["inventario cocina", "inventario de cocina", "s cocina", "s.cocina", "utensilios cocina", "compras cocina"],
    "PAGO POR AGUA DULCE HIELO ETC.": ["agua", "agua dulce", "hielo", "agua e hielo", "agua y hielo", "compra de agua", "compra de hielo"],
    "VISITAS JUAN MANUEL": ["visitas", "juan manuel", "visita juan manuel", "pago visitas"],
    "TRAVEL/SLIP MARINA": ["marina", "slip", "travel slip", "travel", "amarre", "muelle", "puesto marina"],
    "COMIDA ANDRES": ["comida", "comidas", "alimentacion", "alimentación", "comida andres", "alimentación andres"],
    "CCSS ANDRES": ["ccss", "seguridad social", "caja", "aporte ccss", "pago ccss"],
    "OTROS/": ["otros", "otros gastos", "gasto varios", "varios", "miscelaneos", "misceláneos"],
}
...
def _vocab_match_score(query, phrase):
    """Puntaje robusto para lenguaje natural y transcripciones imperfectas."""
    q = norm(query)
    a = norm(phrase)
    if not q or not a:
        return 0.0
    if a in q:
        return 1.0 if len(a.split()) >= 2 else 0.82
    qt, at = _token_set(q), _token_set(a)
    if not at:
        return 0.0
    overlap = len(qt & at) / len(at)
    close = SequenceMatcher(None, q, a).ratio()
    # Para palabras sueltas, exigir similitud alta para evitar falsos positivos.
    if len(at) == 1:
        return max(overlap * 0.82, close * 0.72) if close >= 0.72 else overlap * 0.82
    return max(overlap * 0.9, close * 0.65)


def _contains_term(text, term):
    """Coincidencia de palabra/frase sin falsos positivos por subcadenas."""
    text = norm(text)
    term = norm(term)
...
def detect_system(p):
    p = norm(p)
    # Reglas de alta prioridad: evitan que una palabra secundaria mande la ruta.
    if any(_contains_term(p, x) for x in ("generador", "cummins", "onan", "grupo electrógeno", "grupo electrogeno")): return "Generador Cummins Onan"
    if any(_contains_term(p, x) for x in ("pata", "patas", "outdrive", "muelas", "pierna de motor", "piernas del motor", "piernas de motor", "aceite de pata", "anodos de pata", "ánodos de pata")): return "Patas Mercury"
    if any(_contains_term(p, x) for x in ("motor", "motores", "mercury", "estribor", "babor", "aceite motores", "filtro de motor")): return "Motores Mercury"

    explicit = [
        ("Combustible", ("combustible", "consumo", "gasolina", "diesel", "diésel", "litros de gasolina", "litros diesel", "galones")),
        ("Inventario", ("inventario", "existencias", "stock", "producto en existencia", "cuanto hay", "cuánto hay")),
        ("Facturas", ("factura", "facturas", "invoice", "recibo", "comprobante fiscal")),
        ("Permisos y seguros", ("permiso", "permisos", "seguro", "seguros", "póliza", "poliza", "garantia", "garantía", "vencimiento", "documentación")),
        ("Bitácora", ("bitacora", "bitácora", "vitacora", "vitácora", "falla", "alarma", "reporte de problema", "evento", "reparacion", "reparación", "pendiente")),
        ("Limpieza", ("limpieza", "aseo", "detergente", "cloro", "desinfectante")),
        ("Finanzas", ("presupuesto", "gasto", "gastamos", "gastado", "costo", "monto", "rubro", "cuanto se gasto", "cuánto se gastó")),
    ]
    for system, aliases in explicit:
        if any(_contains_term(p, x) for x in aliases):
            return system

    # Rubros financieros: permiten reconocer "caja", "amarre", "pieza",
    # "sueldo", etc., aunque el usuario nunca diga "presupuesto".
    for rubric, aliases in RUBRIC_VOCABULARY.items():
        if any(_vocab_match_score(p, alias) >= 0.82 for alias in aliases):
            return "Finanzas"

    # Segunda capa: diccionario amplio con coincidencia aproximada.
    candidates = []
    for sheet, cfg in SHEET_VOCABULARY.items():
        score = max((_vocab_match_score(p, a) for a in cfg["aliases"]), default=0.0)
        if score:
            candidates.append((score, cfg["system"], sheet))
    if candidates:
...
def detect_intent(p):
    ...
    # Consumo operativo: "litros", "galones" y "consumo" apuntan a la hoja
    # de combustible cuando la pregunta no está pidiendo un monto financiero.
    fuel_terms = ("combustible", "gasolina", "diesel", "diésel", "litros", "galones", "consumo")
    financial_context = ("gasto", "gastamos", "gastado", "costo", "presupuesto", "presupuestado", "pago", "pagamos", "precio", "valor", "importe", "dinero")
    if any(x in p for x in fuel_terms) and not any(x in p for x in financial_context):
        return "combustible"

    # Finanzas: primero identificar un rubro, pero separar INVENTARIO operativo
    # de INVENTARIO.COCINA del presupuesto.
    finance_words = ("cuanto", "gasto", "gaste", "gastamos", "costo", "presupuesto", "presupuestado", "gastado", "gastó", "pago", "pagamos", "pague", "pagué", "precio", "valor", "importe", "cuanto se gasto")
    ...
    if any(x in p for x in ("horas", "horometro", "horómetro", "horimetro", "horimetro", "horas de trabajo", "horas trabajadas")): return "horometros"
    # El habla puede transcribir "horómetro" como "hora", "ora" o incluso
    # "horas del motor". El vínculo con motor/generador es suficiente.
    if (any(x in p for x in ("hora", "horas", "ora")) and
        any(x in p for x in ("motor", "motores", "mercury", "estribor", "babor", "generador", "cummins", "onan"))):
        return "horometros"

    # Fallback semántico: si la pregunta contiene vocabulario característico
    # de una hoja, inferimos la operación más probable sin exigir una palabra
    # exacta. Esto evita respuestas "no entendí" por una sola variante.
    system = detect_system(p)
    if system == "Bitácora": return "bitacora"
    if system == "Inventario": return "inventario"
    if system == "Combustible": return "combustible"
    if system == "Facturas": return "facturas"
    if system == "Limpieza": return "limpieza"
    if system == "Permisos y seguros": return "documentos"
    if system == "Finanzas": return "gasto_anual" if not find_month(p) else "gasto_mensual"
    if system in ("Motores Mercury", "Patas Mercury", "Generador Cummins Onan") and any(x in p for x in ("servicio", "mantenimiento", "cambio", "aceite", "filtro", "anodo", "ánodo", "kit", "proximo", "próximo", "toca", "intervalo")):
...
def build_dynamic_vocabulary(data):
    """Amplía el diccionario con etiquetas reales presentes en el Excel.

    Solo incorpora nombres/etiquetas cortas útiles para comprender preguntas;
    no copia fotografías ni valores monetarios a este diccionario.
    """
    vocab = {sheet: list(cfg["aliases"]) for sheet, cfg in SHEET_VOCABULARY.items()}
    for sheet, sheet_data in (data.get("sheets") or {}).items():
        target = next((known for known in vocab if norm(known) == norm(sheet)), None)
        if target is None:
            continue
        ...
        for row in rows[:8]:
            ...
            for value in row:
                ...
                if text and len(text) <= 80 and not re.fullmatch(r"[\d\W_]+", text):
                    vocab[target].append(text)

    for rec in data.get("inventory_records", [])[:500]:
        vals = rec.get("values") or []
        if len(vals) > 1 and vals[1] not in (None, ""):
            text = str(vals[1]).strip()
            if len(text) <= 80:
                vocab.setdefault("INVENTARIO", []).append(text)

    for rec in data.get("maintenance_records", []):
        text = str((rec.get("data") or {}).get("mantenimiento") or "").strip()
        if text and len(text) <= 180:
            vocab.setdefault("CHECKLIST MANT. PREVENTIVO", []).append(text)

    for rec in data.get("budget_records", [])[:150]:
        text = _budget_name(rec) if "_budget_name" in globals() else ""
        if text and len(text) <= 100:
            vocab.setdefault("PRESUPUESTO TIARA 2026", []).append(text)

    return {k: list(dict.fromkeys(v)) for k, v in vocab.items()}


def think(question, data=None):
    p = norm(question)
    system = detect_system(p)
    intent = detect_intent(p)
    vocab_map = build_dynamic_vocabulary(data or {}) if data else {k: v["aliases"] for k, v in SHEET_VOCABULARY.items()}
    matched = []
    for sheet, aliases in vocab_map.items():
        for alias in aliases:
            score = _vocab_match_score(p, alias)
            if score >= 0.78:
                matched.append((score, sheet, alias))
    matched.sort(reverse=True)

    # Si el diccionario encuentra una etiqueta concreta del Excel, esa evidencia
    # puede corregir una ruta demasiado genérica (ej. "abridor de botellas" ->
    # INVENTARIO). No sustituye las reglas de alta prioridad de motores/generador.
    if matched and system in ("General", "Limpieza"):
        top_score, top_sheet, _ = matched[0]
        sheet_system = SHEET_VOCABULARY.get(top_sheet, {}).get("system", "General")
        if top_score >= 0.90 and sheet_system != "General":
            system = sheet_system

    # La etiqueta concreta encontrada en el Excel puede corregir también una
    # intención secundaria provocada por una coincidencia débil (por ejemplo,
    # "abridor de botellas" no debe convertirse en limpieza solo porque
    # "bote" aparezca en otra hoja).
    if matched:
        top_score, top_sheet, _ = matched[0]
        if top_score >= 0.90:
            if top_sheet == "INVENTARIO" and intent not in ("gasto_mensual", "gasto_anual", "documentos"):
                intent = "inventario"
            elif top_sheet == "VITACORA BARCO EN LINEA" and intent == "general":
                intent = "bitacora"
            elif top_sheet == "CONSUMO COMBUSTIBLE " and intent == "general":
                intent = "combustible"
            elif top_sheet == "FACTURAS 2026..." and intent == "general":
                intent = "facturas"
            elif top_sheet == "S.LIMPIEZA & OTROS" and intent == "general":
                intent = "limpieza"
            elif top_sheet == "PERMISOS & SEGUROS" and intent == "general":
                intent = "documentos"
            elif top_sheet == "CHECKLIST MANT. PREVENTIVO" and intent == "general":
                intent = "mantenimiento"

    # Si la intención quedó genérica, usamos la hoja identificada como segunda
    # capa para escoger la operación más natural.
    if intent == "general":
        if system == "Bitácora": intent = "bitacora"
        elif system == "Inventario": intent = "inventario"
        elif system == "Combustible": intent = "combustible"
        elif system == "Facturas": intent = "facturas"
        elif system == "Limpieza": intent = "limpieza"
        elif system == "Permisos y seguros": intent = "documentos"
        elif system == "Finanzas": intent = "gasto_mensual" if find_month(p) else "gasto_anual"
        elif system in ("Motores Mercury", "Patas Mercury", "Generador Cummins Onan"):
            if any(_contains_term(p, x) for x in ("estado", "situacion", "situación", "como anda", "cómo anda", "como esta", "cómo está")):
                intent = "estado_general"
            elif any(_contains_term(p, x) for x in ("mantenimiento", "servicio", "cambio", "aceite", "filtro", "anodo", "ánodo", "kit", "proximo", "próximo", "toca")):
                intent = "mantenimiento"
    ...
    return {
        ...
        "year": find_year(p),
        "normalized": p,
        "vocabulary": [{"sheet": s, "term": a, "score": round(sc, 2)} for sc, s, a in matched[:8]],
    }
...
<style>
/* =========================================================
   AGENTE TIARA — interfaz móvil estilo chat marino
   ========================================================= */
html, body, [data-testid="stAppViewContainer"], [data-testid="stApp"] {
    background: #07101a !important;
    color: #eef5fa !important;
}
[data-testid="stHeader"] { background: #07101a !important; }
...
.tiara-brand { display:flex; align-items:center; gap:14px; }
.tiara-boat { font-size: 2.8rem; line-height:1; }
.tiara-title {
    margin:0;
...
.tiara-menu { font-size:2.6rem; color:#f3f7fa; line-height:1; }

/* Banner */
.hero {
    background: linear-gradient(135deg, #0b3958 0%, #075b91 100%);
    border-radius: 25px;
    padding: 25px 28px;
    margin-bottom: 28px;
    border: 1px solid rgba(39,143,203,.35);
...
/* Quick buttons */
div.stButton > button {
    background: linear-gradient(180deg,#111b27 0%,#0e1823 100%) !important;
    ...
    box-shadow: inset 0 1px 0 rgba(255,255,255,.025), 0 4px 14px rgba(0,0,0,.13);
}
...
/* En teléfonos solo las filas de consultas rápidas conservan 2 columnas.
   El compositor tiene su propio contenedor y no se ve afectado. */
@media (max-width: 700px) {
    .st-key-tiara_quick_row div[data-testid="stHorizontalBlock"] {
        flex-wrap: nowrap !important;
        gap: 12px !important;
    }
    .st-key-tiara_quick_row div[data-testid="stHorizontalBlock"] > div {
...
.tiara-user-bubble {
    background: linear-gradient(135deg,#0b5db5,#0b72d8);
    border:1px solid #1598e5;
    border-radius:20px 20px 6px 20px;
    padding:14px 18px;
    margin: 18px 68px 12px auto;
    max-width:78%;
    font-size:1.05rem;
    color:white;
    box-shadow:0 7px 18px rgba(0,60,130,.22);
}
.tiara-answer-card {
    background: linear-gradient(180deg,#0f1b28 0%,#0d1722 100%);
...
/* =========================================================
   Compositor único: texto + micrófono + envío
   ========================================================= */
.tiara-composer {
    background: linear-gradient(180deg,#0b1825 0%,#091521 100%);
...
[data-testid="stTextInput"] input::placeholder { color:#93a9ba !important; }

/* Micrófono: se mantiene como widget real de Streamlit.
   Al grabar, el audio queda guardado en el estado del widget y puede
   enviarse con el mismo botón de la derecha. */
...
[data-testid="stAudioInput"] button {
    ...
    border:2px solid #5b7184 !important;
    color:#f4f8fb !important;
}
[data-testid="stAudioInput"] label { display:none !important; }

/* Fila del compositor. Nunca se apila en móvil. */
.st-key-tiara_composer_row {
    width:100% !important;
}
.st-key-tiara_composer_row div[data-testid="stHorizontalBlock"] {
...
.st-key-tiara_composer_row div[data-testid="stHorizontalBlock"] > div {
    min-width:0 !important;
}
.st-key-tiara_composer_row div[data-testid="stHorizontalBlock"] > div:nth-child(1) {
    flex:0 0 58px !important;
    width:58px !important;
}
.st-key-tiara_composer_row div[data-testid="stHorizontalBlock"] > div:nth-child(2) {
    flex:1 1 auto !important;
    width:auto !important;
}
.st-key-tiara_composer_row div[data-testid="stHorizontalBlock"] > div:nth-child(3) {
    flex:0 0 58px !important;
    width:58px !important;
}
.st-key-tiara_composer_row [data-testid="stTextInput"] {
    width:100% !important;
    margin:0 !important;
}
.st-key-tiara_composer_row [data-testid="stTextInput"] > div > div {
    height:58px !important;
    min-height:58px !important;
    border-radius:30px !important;
    background:#152333 !important;
...
.st-key-tiara_composer_row [data-testid="stTextInput"] input {
    padding-left:16px !important;
    padding-right:12px !important;
}

/* Enviar: botón real de Streamlit, no submit de formulario. */
.st-key-tiara_composer_row div.stButton > button {
    width:58px !important;
    min-width:58px !important;
    max-width:58px !important;
    height:58px !important;
    ...
    padding:0 !important;
    border-radius:50% !important;
    background:linear-gradient(145deg,#159df0,#0878df) !important;
    color:#fff !important;
    border:1px solid #44b8ff !important;
    font-size:1.55rem !important;
    box-shadow:0 5px 16px rgba(0,111,210,.28);
}
.st-key-tiara_composer_row div.stButton > button:hover {
    transform:none !important;
}

/* El contenedor visual no debe añadir una segunda caja. */
[data-testid="stForm"] {
    background:transparent !important;
    border:0 !important;
    padding:0 !important;
...
[data-testid="stForm"] label { color:#9fb4c3 !important; }

/* Sidebar conserva su funcionalidad */
[data-testid="stSidebar"] {
    background:#0a131e !important;
}
textarea, input { color:#f2f5f7 !important; }
[data-testid="stMarkdownContainer"] p, [data-testid="stMarkdownContainer"] li { color:#e7edf2; }
small, .stCaption { color:#9da9b5 !important; }

/* En pantallas pequeñas, dos columnas de botones y compositor compacto */
@media (max-width: 700px) {
    .block-container { padding-left:.75rem !important; padding-right:.75rem !important; }
    .tiara-header { margin-bottom:15px; padding-top:10px; min-height:72px; overflow:visible !important; }
    .tiara-brand { min-width:0; flex:1 1 auto; }
    .tiara-boat { font-size:2.15rem; flex:0 0 auto; }
    .tiara-title { font-size:1.55rem; line-height:1.05; white-space:nowrap; }
    .tiara-subtitle { font-size:.61rem; letter-spacing:.16em; white-space:nowrap; }
    .tiara-menu { font-size:2.15rem; flex:0 0 auto; }
    .hero { padding:20px 20px; border-radius:22px; }
    .hero p { font-size:1rem; line-height:1.6; }
    div.stButton > button { min-height:66px !important; font-size:1rem !important; }
    .tiara-user-bubble { margin-right:12px; max-width:88%; }
    .tiara-answer-card { margin-left:0; }
    .tiara-composer { border-radius:22px; padding:10px 10px 11px; }
    .st-key-tiara_composer_row div[data-testid="stHorizontalBlock"] { gap:8px !important; }
    .st-key-tiara_composer_row div[data-testid="stHorizontalBlock"] > div:nth-child(1),
    .st-key-tiara_composer_row div[data-testid="stHorizontalBlock"] > div:nth-child(3) {
        flex:0 0 58px !important; width:58px !important;
    }
    .st-key-tiara_composer_row [data-testid="stTextInput"] input { font-size:.98rem !important; }
}
</style>
...
<div class="tiara-header">
  <div class="tiara-brand">
    ...
    <div>
      <div class="tiara-title">AGENTE <span>TIARA</span></div>
      <div class="tiara-subtitle">TU ASISTENTE DE CONFIANZA</div>
    </div>
  </div>
  <div class="tiara-menu">☰</div>
</div>
<div class="hero">
...
# ============================================================
# CONSULTAS RÁPIDAS — 2 + 2 + 1
# ============================================================
quick = None
with st.container(key="tiara_quick_row"):
    q1, q2 = st.columns(2, gap="small")
    with q1:
        if st.button("🛥️ Estado general", use_container_width=True, key="quick_estado"):
            quick = "Dame el estado general del barco."
    with q2:
        if st.button("🔧 Motores", use_container_width=True, key="quick_motores"):
            quick = "¿Qué mantenimiento les toca próximamente a los motores?"

    q3, q4 = st.columns(2, gap="small")
    with q3:
        if st.button("🔩 Kit 300 h", use_container_width=True, key="quick_kit"):
            quick = "¿Cuándo toca el próximo kit de 300 horas de los motores?"
    with q4:
        if st.button("⚙️ Patas", use_container_width=True, key="quick_patas"):
            quick = "¿Cuándo toca el próximo cambio de aceite de las patas?"

    if st.button("🔌 Generador", use_container_width=True, key="quick_generador"):
        quick = "¿Cuándo toca el próximo cambio de aceite al generador?"

# ============================================================
# COMPOSITOR ÚNICO Y FUNCIONAL — TEXTO + VOZ + ENVÍO
# No usamos st.form: así la grabación del micrófono se actualiza
# inmediatamente y el botón puede leer tanto texto como audio.
# ============================================================
st.markdown('<div class="tiara-composer">', unsafe_allow_html=True)
with st.container(key="tiara_composer_row"):
    c_voice, c_text, c_send = st.columns([0.9, 6.8, 0.9], gap="small", vertical_alignment="center")
    ...
    with c_send:
        send_question = st.button(
            ...
            help="Enviar al Agente Tiara",
            type="primary",
            use_container_width=True,
        )

st.markdown('<div class="tiara-composer-help">Puedes escribir tu pregunta o tocar el micrófono y hablar. Luego pulsa enviar.</div>', unsafe_allow_html=True)
...
if send_question:
    ...
    if typed_question:
        ...
        if audio_signature != st.session_state.voice_audio_signature:
            with st.spinner("🎙️ Transcribiendo y consultando al Agente Tiara…"):
            ...
            st.info("Esta grabación ya fue procesada. Haz una nueva grabación para enviar otra pregunta.")
    else:
        st.info("Escribe una pregunta o pulsa el micrófono para hablar.")
...
if st.session_state.last_question:
    st.markdown(
        f'<div class="tiara-user-bubble">{st.session_state.last_question}</div>',
        unsafe_allow_html=True,
    )
    st.markdown('<div class="tiara-answer-card">', unsafe_allow_html=True)
