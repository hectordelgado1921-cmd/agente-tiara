...
def _analyze_embedded_images(images):
    ...
    # Prioridad: las fotos de PERMISOS & SEGUROS deben analizarse primero.
    # Un Excel puede contener cientos de imágenes (logos, iconos, fotos de otras hojas);
    # no debemos gastar el cupo de visión en ellas antes de llegar a los documentos.
    target_sheet = "permisos & seguros"
    priority = [im for im in images if norm(im.get("sheet", "")) == target_sheet]
    priority_ids = {id(im) for im in priority}
    remaining = [im for im in images if id(im) not in priority_ids]
    ordered_images = priority + remaining

    analyzed = []
    max_images = max(30, len(priority))
    for im in ordered_images[:max_images]:
        ...
        try:
            ...
            prompt = (
                "Eres el módulo de visión documental del Agente Tiara. Esta imagen "
                "puede estar girada 90, 180 o 270 grados: determina la orientación "
                "correcta y léela como si estuviera derecha. Analiza con máxima "
                "atención documentos de permisos, registros, pólizas, seguros, "
                "garantías y certificados de una embarcación. Extrae SOLO información "
                "realmente visible. Es MUY IMPORTANTE detectar fechas de vencimiento: "
                "busca EXPIRES, EXPIRATION, VALID UNTIL, VIGENCIA HASTA, FECHA DE "
                "VENCIMIENTO, COVERAGE ENDS, ENDS, RENEWAL y fechas impresas en "
                "stickers, sellos o etiquetas. No confundas emisión/inicio con vencimiento.\n\n"
                "Devuelve exactamente estos campos, uno por línea:\n"
                "TIPO_DOCUMENTO: ...\n"
                "NOMBRE_EMBARCACION_PERSONA_EMPRESA: ...\n"
                "NUMERO_POLIZA_REGISTRO: ...\n"
                "FECHA_EMISION: YYYY-MM-DD o NO LEGIBLE\n"
                "FECHA_VENCIMIENTO: YYYY-MM-DD o NO LEGIBLE\n"
                "ASEGURADORA_AUTORIDAD: ...\n"
                "DATOS_IMPORTANTES: ...\n"
                "Si solo permite mes/año, usa YYYY-MM. No inventes ni deduzcas una fecha "
                "que no aparezca explícitamente en la imagen."
            )
    ...
    analyzed_count = min(len(ordered_images), max_images)
    return analyzed, {
        "enabled": True,
        "analyzed": analyzed_count,
        "total": len(images),
        "priority_sheet": "PERMISOS & SEGUROS",
...
def _vision_expiry_dates(data):
    """Extrae fechas de vencimiento explícitas desde el análisis visual."""
    ...
    for im in data.get("images", []):
        ...
        m = re.search(r"FECHA[_ ]?VENCIMIENTO\s*:\s*(\d{4}-\d{2}(?:-\d{2})?)", analysis, re.I)
        if not m:
            continue
        raw = m.group(1)
        try:
            dt = _dt.date.fromisoformat(raw if len(raw) == 10 else raw + "-01")
...
def answer_photo_expiry(data, question):
    ...
    if not found:
        return (
            ...
            "No encontré una fecha de vencimiento explícitamente legible y "
            "estructurada en los análisis visuales guardados. Primero hay que "
            "volver a procesar las fotografías de **PERMISOS & SEGUROS** con la "
            "lectura enfocada en fechas."
        )
    import datetime as _dt
...
def run_agent(data, question, context=None):
    ...
    if intent == "documentos":
        # Vencimientos y consultas sobre documentos deben usar la lectura visual.
        if any(x in p for x in (
            "vencimiento", "vence", "expira", "expiracion", "expiración",
            "fecha limite", "fecha límite", "vigencia hasta", "foto",
            "fotografia", "fotografía", "imagen", "que dice", "qué dice",
            "lee", "leer", "muestra"
        )):
