...
def _budget_sheet_records_direct(data):
    """Fuente de presupuesto para SALIDAS BARCO y su cruce.

    Prioriza la hoja PRESUPUESTO TIARA 2026. Si el snapshot de Supabase
    pertenece a una versión anterior y no trae sheets.records, usa los
    budget_records ya persistidos antes de intentar reconstruir desde Excel.
    """
    sheet = data.get("sheets", {}).get("PRESUPUESTO TIARA 2026", {}) or {}
    records = sheet.get("records") or []
    if records:
        return records

    fallback = data.get("budget_records") or []
    if fallback:
        return fallback

    return _budget_rows_from_excel_values(data)


def _salidas_barco_record(data):
    """Localiza la fila oficial SALIDAS BARCO del presupuesto.

    La fuente principal es la hoja PRESUPUESTO TIARA 2026. Como respaldo,
    también acepta budget_records, porque el snapshot persistido en Supabase
    de versiones anteriores puede conservar los registros del presupuesto
    aunque no conserve la estructura completa de sheets.records.
    """
    records = _budget_sheet_records_direct(data)
    for rec in records:
        if _budget_name_matches(_budget_name(rec), "SALIDAS BARCO"):
            return rec

    # Compatibilidad con snapshots antiguos de Supabase.
    fallback = data.get("budget_records") or []
    for rec in fallback:
        if _budget_name_matches(_budget_name(rec), "SALIDAS BARCO"):
            return rec
...
def _salidas_barco_value(data, year, month=None):
    """Obtiene SALIDAS BARCO exclusivamente de PRESUPUESTO TIARA 2026."""
    ...
    d = rec.get("data") or {}
    # Primero el TOTAL oficial asociado a la fila del rubro.
    for key, value in d.items():
        if norm(key).replace(" ", "") == f"total{year}":
            return _budget_number(value)

    # Compatibilidad con el formato donde el TOTAL está en la fila siguiente.
    source_row = rec.get("source_row")
    try:
    ...
    if source_row is not None:
        for candidate in _budget_sheet_records_direct(data):
            try:
            ...
            if row != source_row + 1:
                continue
            for key, value in (candidate.get("data") or {}).items():
                if norm(key).replace(" ", "") == f"total{year}":
                    return _budget_number(value)
    return None
...
def _combustible_budget_for_salidas(data, year, month=None):
    ...
    if not matches:
        # Compatibilidad con snapshots antiguos de Supabase que conservan
        # budget_records pero no sheets.records.
        fallback = data.get("budget_records") or []
        matches = [r for r in fallback if _budget_name_matches(_budget_name(r), "COMSUMO COMBUSTIBLE") or _budget_name_matches(_budget_name(r), "CONSUMO COMBUSTIBLE")]
    if not matches:
        return None
