...
/* Micrófono nativo de Streamlit: no se reemplaza por HTML falso. */
...
# ============================================================
# CHAT — TEXTO + MICRÓFONO + ENVIAR
# ============================================================
st.markdown('<div class="tiara-chat-space"></div>', unsafe_allow_html=True)

# No usamos st.form para el audio: st.audio_input es un widget de archivo
# y necesitamos que su valor quede disponible en el backend inmediatamente
# despues de terminar la grabacion. El boton normal solo dispara el procesamiento.
with st.container(key="tiara_composer"):
    c_voice, c_text, c_send = st.columns([1.15, 6.5, 1.25], vertical_alignment="bottom")
...
st.markdown(
    '<div class="tiara-help">Puedes escribir tu pregunta o tocar el micrófono y hablar. Luego pulsa enviar.</div>',
    unsafe_allow_html=True,
)
...
if send_question:
    ...
    if typed_question:
        # Pregunta escrita: va directamente al mismo cerebro del agente.
        quick = typed_question

    elif voice_audio is not None:
        # Pregunta hablada: transcribir primero y después enviarla al mismo
        # run_agent(), exactamente igual que una pregunta escrita.
        ...
        if audio_signature != st.session_state.voice_audio_signature:
            ...
            if voice_question:
                # Marcamos el audio como procesado SOLO despues de obtener una
                # transcripcion valida. Si Gemini falla, el usuario puede pulsar
                # Enviar de nuevo sin tener que volver a grabar.
