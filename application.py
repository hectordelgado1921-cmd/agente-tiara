...
def build_from_excel(path: Path):
    ...
    data["images"] = _extract_excel_images(path)
    return data
...
def detect_intent(p):
    ...
    if any(x in p for x in ("foto", "fotografia", "fotografía", "imagen", "documento fotografiado", "que dice la foto", "qué dice la foto")): return "fotografias"
    if any(x in p for x in ("limpieza", "limpiar", "aseo")): return "limpieza"
...
def answer_photos(data, question):
    ...
    if not images:
        return "No encontré fotografías incrustadas en el Excel. Si la foto está dentro de una hoja, necesito que el archivo conserve la imagen incrustada."
    ...
    lines = [f"### Fotografías detectadas — {len(images)}"]
    for im in chosen[:12]:
        loc = f"hoja **{im['sheet']}**"
        if im.get("row"): loc += f", fila {im['row']}"
        lines.append(f"- Fotografía {im['index']}: {loc} ({im.get('format','imagen')}).")
    lines.append("\nLas fotografías quedaron incorporadas como evidencia visual. El cerebro puede localizarlas y mostrarlas; para describir visualmente su contenido con IA multimodal hace falta conectar un modelo de visión, no solo leer las celdas del Excel.")
    return "\n".join(lines)
