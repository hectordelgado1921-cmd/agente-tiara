...
def load_supabase_data():
    ...
    try:
        rows = _supabase_request(
            ...
            "",
            query=(
                "?select=data,source_filename,source_hash,version,created_at"
        ...
        if row.get("version") and not meta.get("version"):
...
def save_supabase_data(data):
    ...
    payload = {
        ...
        "version": str(meta.get("version", APP_VERSION)),
        "is_active": True,
        "data": data,
    }
    _supabase_request("POST", "", payload=payload, query="?select=id")
