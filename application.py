...
def detect_intent(p):
    ...
    # SALIDAS BARCO: cantidad operativa/financiera. Si la pregunta relaciona
    # combustible con salidas, entra al análisis financiero específico.
    if (any(x in p for x in (
        "costo por salida", "costo de combustible por salida",
        "gasto por salida", "combustible por salida",
        "costo de cada salida", "costo de cada salida del barco",
        "combustible entre las salidas", "gasto de combustible entre las salidas"
    )) and any(x in p for x in ("salida", "salidas", "barco"))):
        return "costo_combustible_salida"

    # SALIDAS BARCO: cantidad operativa/financiera. Se reconoce antes de la
    # lógica monetaria para no tratar el número de salidas como dólares.
    if any(x in p for x in (
        "salidas barco", "salida barco", "salidas del barco",
        "salidas de barco", "salida de barco", "cuantas salidas",
        "cuántas salidas", "cantidad de salidas", "numero de salidas",
        "número de salidas", "numero de salidas del barco",
        "número de salidas del barco"
    )):
        return "salidas_barco"
...
def identify_rubric(p):
    ...
    aliases = [
        ...
        (("salidas barco", "salida barco", "salidas del barco", "salida del barco", "salidas de barco", "salida de barco"), "SALIDAS BARCO"),
...
def _find_salidas_barco_record(data):
    """Busca exclusivamente la fila RUBROS = SALIDAS BARCO."""
    for rec in _budget_rows_from_excel_values(data):
...
def salidas_barco_value(data, year, month=None):
    """Obtiene SALIDAS BARCO por mes o el TOTAL oficial del año.

    El total anual está en la fila TOTAL inmediatamente posterior a SALIDAS BARCO
    en el presupuesto actual. No se consultan ni suman filas de OBSERVACIONES.
    """
    rec = _find_salidas_barco_record(data)
    if not rec:
        return None
    if month is not None:
        return _budget_dict_value(rec, year, month)

    # Preferir el TOTAL oficial asociado a la fila SALIDAS BARCO.
    source_row = rec.get("source_row")
    if source_row is not None:
        for candidate in _budget_rows_from_excel_values(data):
            if candidate.get("source_row") == source_row + 1:
                d = candidate.get("data") or {}
                for key, value in d.items():
                    if norm(key).replace(" ", "") == f"total{year}":
                        return _budget_number(value)

    # Respaldo si el formato cambia y el total viene en la propia fila.
    d = rec.get("data") or {}
    for key, value in d.items():
        if norm(key).replace(" ", "") == f"total{year}":
            return _budget_number(value)
...
def _fuel_budget_value(data, year, month=None):
    """Obtiene combustible del presupuesto para el período solicitado."""
    if month:
...
def answer_costo_combustible_salida(data, thought):
    """Relaciona exclusivamente combustible con SALIDAS BARCO por período."""
    p = thought["normalized"]
    month = thought.get("month")
    years = sorted({int(y) for y in re.findall(r"\b(20\d{2})\b", p)})
    if not years:
        years = [thought.get("year") or 2026]
    if len(years) > 2:
        years = years[-2:]

    results = []
    for year in years:
        fuel = _fuel_budget_value(data, year, month)
        outings = salidas_barco_value(data, year, month)
        if fuel is None or outings is None or outings <= 0:
            results.append((year, fuel, outings, None))
        else:
            results.append((year, fuel, outings, fuel / outings))

    if len(results) == 1:
        year, fuel, outings, cost = results[0]
        period = f"{MONTH_NAMES[month].capitalize()} {year}" if month else str(year)
        if fuel is None or outings is None or outings <= 0:
            return (f"### Costo de combustible por salida\n"
                    f"**Período:** {period}\n"
                    f"**Gasto de combustible:** {money(fuel) if fuel is not None else 'NO DETERMINADO'}\n"
                    f"**Salidas:** {int(outings) if outings is not None and float(outings).is_integer() else outings if outings is not None else 'NO DETERMINADO'}\n"
                    f"**Costo por salida:** NO DETERMINADO\n"
                    f"**Fuente:** PRESUPUESTO TIARA 2026 — COMBUSTIBLE + SALIDAS BARCO")
        return (f"### Costo de combustible por salida\n"
                f"**Período:** {period}\n"
                f"**Gasto de combustible:** {money(fuel)}\n"
                f"**Salidas:** {int(outings) if float(outings).is_integer() else outings}\n"
                f"**Costo de combustible por salida:** {money(cost)}\n"
                f"**Fuente:** PRESUPUESTO TIARA 2026 — COMBUSTIBLE + SALIDAS BARCO")

    lines = ["### Costo de combustible por salida"]
    for year, fuel, outings, cost in results:
        period = f"{MONTH_NAMES[month].capitalize()} {year}" if month else str(year)
        lines.append(f"**{period}:** {money(cost) if cost is not None else 'NO DETERMINADO'} por salida")
    a, b = results[0], results[1]
    if a[3] is not None and b[3] is not None and b[3] != 0:
        diff = b[3] - a[3]
        pct = diff / a[3] * 100 if a[3] != 0 else None
        lines.append(f"**Diferencia:** {money(diff)}")
        lines.append(f"**Variación:** {pct:.2f}%" if pct is not None else "**Variación:** NO DETERMINADO")
    else:
        lines.append("**Diferencia:** NO DETERMINADO")
        lines.append("**Variación:** NO DETERMINADO")
    lines.append("**Fuente:** PRESUPUESTO TIARA 2026 — COMBUSTIBLE + SALIDAS BARCO")
    return "\n".join(lines)


def answer_salidas_barco(data, thought):
    """Consulta simple de SALIDAS BARCO. No usa OBSERVACIONES ni las suma."""
    year = thought.get("year") or 2026
    month = thought.get("month")

    if month:
        value = salidas_barco_value(data, year, month)
        return (
            f"### Salidas del barco\n"
            f"**Mes:** {MONTH_NAMES[month].capitalize()} {year}\n"
            f"**Salidas registradas:** {int(value) if value is not None and float(value).is_integer() else value if value is not None else 'NO DETERMINADO'}\n"
            f"**Fuente:** PRESUPUESTO TIARA 2026 — SALIDAS BARCO\n"
            f"**Estado:** {'CONFIRMADO' if value is not None else 'NO DETERMINADO'}"
        )

    value = salidas_barco_value(data, year)
    return (
        f"### Salidas del barco\n"
        f"**Año:** {year}\n"
        f"**Salidas registradas:** {int(value) if value is not None and float(value).is_integer() else value if value is not None else 'NO DETERMINADO'}\n"
...
def run_agent(data, question, context=None):
    ...
    if intent == "salidas_barco":
        return answer_salidas_barco(data, thought)
    if intent == "costo_combustible_salida":
        return answer_costo_combustible_salida(data, thought)
