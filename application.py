...
/* Las columnas de la pantalla principal no se apilan en móvil.
   Esto conserva 2 + 2 + 1 en accesos rápidos y una sola fila
   proporcional para voz + texto + enviar. */
...
@media (max-width: 700px) {
    ...
    /* Una sola fila compacta: micrófono | texto | enviar. */
...
with st.form("tiara_unified_composer", clear_on_submit=True, border=False):
    # Proporción compacta para teléfono: micrófono | texto | enviar.
    c_voice, c_text, c_send = st.columns([1.35, 6.2, 1.35], vertical_alignment="bottom", gap="small")
