...
APP_VERSION = "2.8.0"
APP_UPDATED = "14/09/2026"  # Fecha de la versión del código; la fecha de carga del Excel es dinámica.
...
def _encode_supabase_snapshot(data):
    """Comprime el snapshot antes de enviarlo a Supabase.

    El Excel puede generar una base JSON grande porque conserva las hojas
    necesarias para consultas detalladas. Gzip reduce drásticamente el
    tamaño del cuerpo HTTP sin cambiar la información que usa el agente.
    """
    raw = json.dumps(
        data,
        ensure_ascii=False,
...
def _decode_supabase_snapshot(value):
    """Descomprime snapshots nuevos y conserva compatibilidad con los antiguos."""
    ...
    if not isinstance(payload, str) or not payload:
        raise ValueError("El snapshot comprimido de Supabase no contiene payload.")
    raw = gzip.decompress(base64.b64decode(payload))
    data = json.loads(raw.decode("utf-8"))
    if not isinstance(data, dict):
        raise ValueError("El snapshot de Supabase no contiene una base de datos válida.")
    return data
...
def load_supabase_data():
    ...
    try:
        ...
        if not rows:
            return None
        row = rows[0]
        data = _decode_supabase_snapshot(row.get("data"))
        if not isinstance(data, dict):
            return None
...
def save_supabase_data(data):
    """Guarda un snapshot completo, comprimido, en Supabase."""
    meta = data.get("metadata", {})
    encoded_data = _encode_supabase_snapshot(data)
    payload = {
...
def build_from_uploaded_excel(uploaded_file):
    ...
    try:
        ...
        data["metadata"]["source_updated_in_app"] = datetime.now(
            APP_TIMEZONE
        ...
        if st.session_state.uploaded_signature != signature:
            ...
            try:
                ...
                if supabase_save_error:
                    # Mostramos el motivo real para poder corregir la última
                    # barrera de Supabase (por ejemplo RLS/permisos), sin
                    # mostrar nunca la URL ni ninguna clave.
                    safe_error = str(supabase_save_error).replace("\n", " ").strip()[:500]
                    st.session_state.upload_message += (
                        f" · ⚠️ Supabase: {safe_error}"
                    )
...
with st.sidebar:
    ...
    # La fecha de actualización de la base cambia cada vez que el usuario
    # carga un Excel. Se guarda dentro del snapshot para que también sobreviva
    # al reinicio de Streamlit y a futuras sesiones desde Supabase.
    ...
    if updated_at:
        try:
            ...
            updated_dt = updated_dt.astimezone(APP_TIMEZONE)
            updated_text = updated_dt.strftime("%d/%m/%Y %H:%M")
        except Exception:
            updated_text = str(updated_at)
    elif meta.get("supabase_created_at"):
        try:
            persisted_dt = datetime.fromisoformat(
                str(meta["supabase_created_at"]).replace("Z", "+00:00")
            ).astimezone(APP_TIMEZONE)
            updated_text = persisted_dt.strftime("%d/%m/%Y %H:%M")
        except Exception:
