...
def _classify_log_system(text):
    """Identifica el sistema al que pertenece la hora de la fila.

    La columna HORAS/MO/GE puede contener horas de motores, patas o generador.
    Por eso la hora solo se asigna cuando el texto del evento identifica de
    forma suficientemente clara el equipo. Si una misma fila mezcla dos
    sistemas, no se inventa una asignación.
    """
    p = norm(text)

    generator_terms = (
        r"\bgenerador\b", r"\bcummins\b", r"\bonan\b",
        r"\bmotor(?:es)?\s+(?:del|de)\s+(?:generador|cummins|onan)\b",
        r"\bgenerador(?:a|es)?\s+cummins\b",
    )
    leg_terms = (r"\bpata\b", r"\bpatas\b", r"\boutdrive\b", r"\bmuelas\b")
    motor_terms = (r"\bmotor\b", r"\bmotores\b", r"\bmercury\b", r"\bestribor\b", r"\bbabor\b")

    generator_hits = sum(bool(re.search(term, p)) for term in generator_terms)
    leg_hits = sum(bool(re.search(term, p)) for term in leg_terms)
    motor_hits = sum(bool(re.search(term, p)) for term in motor_terms)

    systems = []
    if generator_hits:
        systems.append(("Generador Cummins Onan", generator_hits))
    if leg_hits:
        systems.append(("Patas Mercury", leg_hits))
    if motor_hits:
        systems.append(("Motores Mercury", motor_hits))

    if not systems:
        return "General"

    # Una fila que realmente menciona más de un sistema es ambigua para una
    # única cifra de HORAS/MO/GE. No asignamos esa cifra a ningún horómetro.
    if len(systems) > 1:
        return "Ambos"
...
def parse_log_record(vals, row_number):
    ...
    # Si una cifra está escrita en la columna de horas y la fila no identifica
    # el sistema, la dejamos sin asignar para no contaminar otro horómetro.
...
def latest_log_hours(data, system):
    """Obtiene el horómetro más reciente de la bitácora para un sistema.

    Se ordena por fecha del reporte y luego por hora. No se usa simplemente
    max(horas), porque la fuente de verdad es el evento más reciente.
    """
    candidates = []
    for rec in log_records(data):
        d = rec.get("data") or {}
        if d.get("sistema") != system:
            continue
        h = d.get("horas")
        if h is None:
            continue
        rd = _log_date(rec)
        candidates.append((rd or date.min, float(h), rec))
    if not candidates:
        return None
...
def detect_system(p):
    if any(x in p for x in ("generador", "cummins", "onan")):
        return "Generador Cummins Onan"
    if any(x in p for x in ("pata", "patas", "outdrive", "muelas")):
        return "Patas Mercury"
    if any(x in p for x in ("motor", "motores", "mercury", "estribor", "babor")):
        return "Motores Mercury"
    if any(x in p for x in ("combustible", "gasolina", "diesel", "diésel")):
        return "Combustible"
    if any(x in p for x in ("inventario", "existencias", "stock", "producto", "vasos", "fender", "chaleco")):
        return "Inventario"
    if any(x in p for x in ("factura", "facturas", "invoice")):
        return "Facturas"
    if any(x in p for x in ("permiso", "permisos", "seguro", "seguros", "garantia", "garantía")):
        return "Permisos y seguros"
    if any(x in p for x in ("bitacora", "bitácora", "evento", "problema", "falla", "alarma", "reparacion", "reparación")):
        return "Bitácora"
    if any(x in p for x in ("presupuesto", "gasto", "gastamos", "cuanto gastamos", "cuánto gastamos", "costo", "costos")):
        return "Finanzas"
    return "General"


def detect_intent(p):
    if any(x in p for x in ("estado general", "estado del barco", "estado completo", "revision general", "revisión general")):
        return "estado_general"

    # Las preguntas financieras tienen prioridad cuando el usuario nombra un
    # rubro. Esto es especialmente importante en preguntas de seguimiento
    # como "en ese mismo mes, cuanto en limpieza?" o "y en generador?".
    # Sin esta prioridad, palabras como "mantenimiento" o "generador" podían
    # enviar la consulta al módulo de mantenimiento en vez del presupuesto.
    finance_words = (
        "cuanto", "cuánto", "gasto", "gasté", "gaste", "gastamos",
        "costo", "costó", "presupuesto", "presupuestado",
        "en ese mes", "en ese mismo mes", "ese mes", "mismo mes",
    )
    if identify_rubric(p) is not None and any(x in p for x in finance_words):
        return "gasto_mensual" if (any(m in p for m in MONTHS) or
                                    any(x in p for x in ("ese mes", "mismo mes", "ese mismo mes"))) else "gasto_anual"

    if any(x in p for x in ("cuanto gastamos", "cuánto gastamos", "cuanto gasto", "cuánto gasto", "gasto en", "gastamos en", "presupuesto")):
        if any(m in p for m in MONTHS):
            return "gasto_mensual"
        return "gasto_anual"
    # Estado de eventos de la bitácora. "Mantenimiento pendiente" sigue
    # perteneciendo al módulo de mantenimiento; los demás pendientes/listos
    # se interpretan como estado de la bitácora.
    if any(x in p for x in ("pendiente", "pendientes", "sin resolver", "listo", "resuelto", "reparado")) and "mantenimiento" not in p:
        return "bitacora"

    if any(x in p for x in ("cuantas veces", "cuántas veces", "cuantos cambios", "cuántos cambios", "cantidad de cambios", "cantidad de mantenimientos")):
        return "cantidad_mantenimiento"
    if any(x in p for x in ("historial completo", "todo el historial", "todas las veces", "todos los cambios", "todos los mantenimientos", "muestrame el historial", "muéstrame el historial")):
        return "historial_completo"
    if any(x in p for x in ("ultimo cambio", "último cambio", "ultimo mantenimiento", "último mantenimiento", "cuando fue", "cuándo fue", "cuando se hizo", "cuándo se hizo", "cuando se cambio", "cuándo se cambió", "ultima vez", "última vez")):
        return "ultimo_mantenimiento"
    if any(x in p for x in ("kit 300", "kit de 300", "300 horas", "servicio 300")):
        return "kit_300"
    if any(x in p for x in ("cuando toca", "cuándo toca", "proximo", "próximo", "faltan horas", "que mantenimiento", "qué mantenimiento", "mantenimiento")):
        return "mantenimiento"
    if any(x in p for x in ("cuanto hay", "cuánto hay", "existencias", "stock", "inventario", "tenemos")):
        return "inventario"
    if any(x in p for x in ("combustible", "gasolina", "diesel", "diésel")):
        return "combustible"
    if any(x in p for x in ("factura", "facturas")):
        return "facturas"
    if any(x in p for x in ("permiso", "seguro", "seguros", "vence", "vencimiento")):
        return "documentos"
    if any(x in p for x in ("bitacora", "bitácora", "evento", "falla", "alarma", "problema")):
        return "bitacora"
    if any(x in p for x in ("horas", "horometro", "horómetro")):
        return "horometros"
    return "general"
...
def current_hours(data):
    result = {}
    # IMPORTANTE: el checklist NO se modifica ni se usa para reemplazar el
    # historial de mantenimientos. Solo sirve como respaldo cuando todavía no
    # existe una lectura en la bitácora. Los horómetros actuales se actualizan
    # con la última lectura identificada en la bitácora.
...
def identify_rubric(p):
    """Identifica cualquier rubro financiero de la base de presupuesto."""
    p = norm(p)

    # Primero las coincidencias de varias palabras, luego las de una palabra.
    # Así "mantenimiento anual" no termina clasificado como mantenimiento
    # de motores y "mantenimiento del generador" conserva su rubro correcto.
    ordered = sorted(RUBRIC_RULES, key=lambda item: len(item[0]), reverse=True)
    for words, rubric in ordered:
        if all(norm(w) in p for w in words):
            return rubric
    return None
...
def answer_general(data, thought):
    ...
    if thought["system"] == "Inventario":
        return answer_inventory(data, thought)
    return ("Puedo consultar mantenimiento, estado del barco, Kit 300 h, combustible, presupuesto/gastos, "
            "inventario, bitácora, facturas y permisos/seguros.\n\n"
            "Prueba, por ejemplo: **¿Cuál fue el último cambio de aceite de los motores?**")
...
with st.sidebar:
    ...
    if uploaded_file is not None:
        # La firma usa el contenido real del archivo, no solo nombre y tamaño.
        # Así, si el Excel cambia pero conserva el mismo nombre/tamaño, la
        # aplicación vuelve a procesarlo y actualiza los horómetros y datos.
        raw_upload = uploaded_file.getvalue()
        signature = (
            uploaded_file.name,
            len(raw_upload),
            hashlib.sha256(raw_upload).hexdigest(),
        )
        ...
        if st.session_state.uploaded_signature != signature:
            ...
            try:
                ...
                # Solo sustituimos la fuente activa cuando el procesamiento
                # terminó correctamente. Si falla, la base anterior queda viva.
...
# Si el usuario subió un Excel, esa es la fuente de verdad de esta sesión.
# Si no, se usa la base incluida en GitHub.
data = st.session_state.uploaded_data or load_data()
