...
# Preguntas por voz: el navegador abre el micrófono, grabamos una pregunta
# corta y Gemini la convierte a texto. Luego usamos exactamente el mismo
# cerebro que para una pregunta escrita; no existe un segundo agente.
...
if voice_audio is not None:
    ...
    if audio_signature != st.session_state.voice_audio_signature:
        ...
        with st.spinner("🎙️ Entendiendo tu pregunta…"):
            voice_question, voice_error = _gemini_audio_transcribe(
                audio_bytes, getattr(voice_audio, "type", None) or "audio/webm"
            )
        if voice_question:
