...
def latest_log_hours(data, system):
    """Obtiene el horómetro actual exclusivamente de la Bitácora.

    Motores y patas comparten el horómetro de motores. El generador usa
    exclusivamente filas cuyo evento identifica al generador. La hora debe
    salir de la misma fila y gana el registro más reciente por FECHA REPORTE
    y, en empate, por número de fila. El Checklist no reemplaza una lectura
    válida de la Bitácora.
    """
    ...
    for rec in log_records(data):
        ...
        classified = d.get("sistema")

        if target in horas_por_sistema:
            h = horas_por_sistema[target]
        elif classified == target:
            h = d.get("horas")
        elif system == "Patas Mercury" and classified == "Patas Mercury":
            h = d.get("horas")
        else:
            continue
        ...
        if target == "Generador Cummins Onan":
            texto = norm(" ".join(str(x) for x in (
                d.get("observacion"), d.get("observaciones")
            ) if x not in (None, "")))
            if not re.search(r"\b(?:generador|cummins|onan)\b", texto):
                continue
...
with st.sidebar:
    ...
    if uploaded_file is not None:
        # La firma evita reconstruir la base en cada rerun mientras el mismo
        # archivo siga seleccionado.
        signature = (
            uploaded_file.name,
            uploaded_file.size,
        )
