...
def detect_financial_operation(p):
    """Identifica la operación financiera solicitada, sin decidir el rubro.

    El rubro sigue viniendo exclusivamente de identify_rubric() y la activación
    definitiva se hace solo después de comprobar que ese rubro existe en
    PRESUPUESTO TIARA 2026.
    """
    p = norm(p)
    if any(x in p for x in (
        "comparar", "compara", "comparacion", "comparación", "versus", "vs",
        "diferencia", "diferencias", "variacion", "variación",
        "aumento", "aumentó", "aumento de", "incremento", "incrementó",
        "disminucion", "disminución", "disminuyo", "disminuyó",
        "subio", "subió", "bajo", "bajó", "crecio", "creció",
        "mismo periodo", "mismo mes", "misma fecha"
    )):
        return "comparacion"
    if any(x in p for x in ("promedio", "promedios", "promedio mensual", "promedio anual")):
        return "promedio"
    if any(x in p for x in (
        "gasto vs presupuesto", "gasto versus presupuesto",
        "gastado vs presupuestado", "gastado versus presupuestado",
        "contra el presupuesto", "contra presupuesto",
        "excedio el presupuesto", "excedió el presupuesto",
...
def _financial_analysis(data, thought):
    """Motor único del Proyecto de Finanzas.

    Se activa únicamente cuando: (1) hay un rubro identificado y (2) ese rubro
    existe en PRESUPUESTO TIARA 2026 y (3) la pregunta contiene una operación
    financiera aprobada.
    """
    ...
    if not rubric or not operation:
        return None
    if _find_budget_record(data, rubric) is None:
        return None
    ...
    if operation == "comparacion":
        ...
        values = _financial_values_for_period(data, rubric, (old_year, new_year), month)
        old_value, new_value = values.get(old_year), values.get(new_year)
        period = MONTH_NAMES[month].capitalize() if month else "año completo"
        lines = [
            "### Análisis financiero — Comparación",
            f"**Rubro:** {rubric}",
            f"**Período:** {period}",
        ...
        difference, percentage = _financial_change(old_value, new_value)
        if difference is None:
            lines.append("**Diferencia:** NO DETERMINADO — falta uno de los valores necesarios.")
            lines.append("**Variación:** NO DETERMINADA")
        else:
            direction = "aumentó" if difference > 0 else "disminuyó" if difference < 0 else "no cambió"
            lines.append(f"**Diferencia:** {money(abs(difference))} — el gasto {direction} de {old_year} a {new_year}.")
            if percentage is None:
                lines.append("**Variación porcentual:** NO DETERMINADA porque el valor del año de referencia es $0.")
            else:
                lines.append(f"**Variación porcentual:** {percentage:+.2f}%")
        if month is None and any(x in p for x in ("misma fecha", "fecha de hoy", "a hoy", "hasta hoy", "al dia de hoy", "al día de hoy")):
            lines.append("**Corte temporal:** el presupuesto está registrado por mes, por lo que la comparación a la fecha de hoy se interpreta hasta el mes actual, no por día.")
            current_month = date.today().month
            ytd_values = {y: sum((budget_value_for_month(data, rubric, y, m) or 0) for m in range(1, current_month + 1) if budget_value_for_month(data, rubric, y, m) is not None) for y in (old_year, new_year)}
            ytd_found = {y: any(budget_value_for_month(data, rubric, y, m) is not None for m in range(1, current_month + 1)) for y in (old_year, new_year)}
            lines.append(f"**Acumulado hasta {MONTH_NAMES[current_month].capitalize()}:** {old_year}: {money(ytd_values[old_year]) if ytd_found[old_year] else 'NO DETERMINADO'} · {new_year}: {money(ytd_values[new_year]) if ytd_found[new_year] else 'NO DETERMINADO'}")
    ...
    if operation == "gasto_vs_presupuesto":
        # En esta versión el presupuesto mensual/anual es la fuente financiera
        # disponible. No se inventa un 'gasto real' si no existe un campo válido.
        return (f"### Análisis financiero — Gasto vs presupuesto\n**Rubro:** {rubric}\n"
                "El presupuesto está identificado, pero no se encontró en esta estructura un valor independiente de gasto real que permita calcular la diferencia sin inventar datos.\n"
                "**Resultado:** NO DETERMINADO")

    if operation == "consumido_vs_gastado":
        return (f"### Análisis financiero — Consumido vs gastado\n**Rubro:** {rubric}\n"
                "Esta operación requiere dos valores independientes (consumido y gastado). La estructura actual no los presenta como dos magnitudes comparables para este rubro.\n"
                "**Resultado:** NO DETERMINADO")
...
def run_agent(data, question, context=None):
    ...
    # Proyecto de Finanzas: se intenta ANTES de las respuestas simples.
    # La activación depende del rubro identificado y de que ese rubro exista
    # realmente en PRESUPUESTO TIARA 2026, además de una operación financiera.
