...
def build_from_uploaded_excel(uploaded_file):
    """Procesa el Excel subido sin exigir que el usuario lo descomprima."""
    suffix = Path(uploaded_file.name).suffix.lower()
    if suffix not in (".xlsx", ".xlsm"):
        raise ValueError("El archivo debe ser Excel .xlsx o .xlsm")

    # Streamlit mantiene el archivo subido en un buffer. Lo copiamos por
    # bloques a un temporal para que openpyxl pueda leerlo en read_only y
    # evitar mantener otra copia gigante en memoria.
    h = hashlib.sha256()
    with tempfile.NamedTemporaryFile(prefix="tiara_", suffix=suffix, delete=False) as tmp:
    ...
    try:
        ...
        data["metadata"]["source_size_mb"] = round(temp_path.stat().st_size / (1024 * 1024), 2)
        return data
    finally:
        try:
...
def current_hours(data):
    ...
    for system, source_row in targets.items():
        match = next((r for r in rows if int(r.get("source_row", -1)) == source_row), None)
        if match:
...
# ============================================================
# El usuario puede reemplazar el Excel sin tocar GitHub. El archivo se
# procesa en la sesión actual; no se muestra el historial del chat.
...
with st.sidebar:
    ...
    uploaded_file = st.file_uploader(
        "Carga aquí el Excel nuevo del barco",
        type=["xlsx", "xlsm"],
        help="El Agente Tiara procesa automáticamente el Excel. No necesitas descomprimirlo ni convertirlo manualmente.",
    ...
    if uploaded_file is not None:
        ...
        if st.session_state.uploaded_signature != signature:
            try:
                with st.spinner("Procesando el Excel y actualizando la base del agente…"):
                    st.session_state.uploaded_data = build_from_uploaded_excel(uploaded_file)
                st.session_state.uploaded_signature = signature
                st.session_state.last_question = ""
                st.session_state.last_answer = ""
                st.session_state.query_context = {"month": None, "year": None}
                st.session_state.upload_message = (
                    f"Excel actualizado: {uploaded_file.name} "
                    f"({uploaded_file.size / (1024 * 1024):.2f} MB)"
                )
            except Exception as exc:
                st.session_state.uploaded_data = None
                st.session_state.uploaded_signature = None
                st.session_state.upload_message = f"No se pudo procesar el Excel: {exc}"
...
with st.sidebar:
    ...
    st.caption(f"Fuente: {meta.get('source_file', 'Excel de septiembre 2026')}")
