...
def _salidas_barco_value(data, year, month=None):
    """Obtiene SALIDAS BARCO sin alterar la lógica histórica.

    2025 conserva exactamente el lector original.
    Para 2026 se usa exclusivamente la fila oficial singular SALIDA BARCO,
    porque existe una fila histórica/plural con el valor 30 que no corresponde
    a los totales oficiales actuales.
    """
    # CORRECCIÓN AISLADA: solamente 2026.
    if year == 2026:
        ...
        if official:
            if month is not None:
                return _budget_dict_value(official, year, month)
            for key, value in (official.get("data") or {}).items():
                if norm(key).replace(" ", "") == "total2026":
                    return _budget_number(value)
        return None

    # 2025: EXACTAMENTE la lógica que ya funcionaba.
