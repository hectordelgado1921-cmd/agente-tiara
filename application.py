...
def identify_rubric(p):
    for words, rubric in RUBRIC_RULES:
...
def budget_value_for_month(data, rubric, year, month):
    ...
    for rec in rows:
        ...
        if norm(name) != norm(rubric):
            continue
        # Layout confirmado por el Excel: ene-26 en índice 3; cada mes ocupa dos columnas.
        if year == 2026:
            col = 3 + (month - 1) * 2
        elif year == 2025:
            col = 4 + (month - 1) * 2
        elif year == 2024:
            # Histórico: bloque comienza en fila 27 y enero-24 es índice 3.
            col = 3 + (month - 1) * 2
        else:
            return None
        if col >= len(vals):
            return None
        return safe_float(vals[col])
    return None
...
def budget_total(data, rubric, year):
    ...
    if not target:
        return None
    if year == 2026:
        return safe_float(target[27])
    if year == 2025:
        return safe_float(target[28])
    if year == 2024:
        # Buscar fila histórica exacta.
        for rec in rows:
            vals = rec.get("values", [])
            if len(vals) >= 30 and norm(str(vals[2] or "")) == norm(rubric):
                return safe_float(vals[27])
    return None


def answer_expenses(data, thought):
    rubric = identify_rubric(thought["normalized"])
    year = thought["year"] or 2026
    if not rubric:
        return "No pude determinar el rubro del gasto."
    month = thought["month"]
    if month:
        value = budget_value_for_month(data, rubric, year, month)
        value = 0 if value is None else value
        return (f"### Gasto mensual\n**Rubro:** {rubric}\n**Mes:** {MONTH_NAMES[month].capitalize()} {year}\n"
                f"**Gasto registrado:** {money(value)}\n**Estado:** CONFIRMADO desde PRESUPUESTO TIARA 2026")
    value = budget_total(data, rubric, year)
    if value is None:
        return f"### Gasto anual\n**Rubro:** {rubric}\n**Año:** {year}\n**Gasto:** NO DETERMINADO"
    return f"### Gasto anual\n**Rubro:** {rubric}\n**Año:** {year}\n**Gasto registrado:** {money(value)}\n**Estado:** CONFIRMADO"
...
def answer_fuel(data, thought):
    ...
    for m in range(1, 13):
        lines.append(f"- **{MONTH_NAMES[m].capitalize()}:** {money(totals[m])}")
...
# Estado de interfaz: NO guardamos historial de preguntas/respuestas.
...
if quick:
    ...
    st.session_state.last_answer = run_agent(data, quick)
