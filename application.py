...
def latest_log_hours(data, system):
    ...
    La Bitácora es la fuente de actualización del horómetro. Se toma el
    evento más reciente por fecha de reporte y, si hay varias filas con la
    misma fecha, se usa la última fila del Excel. Nunca se mezclan horas de
    motores, patas y generador.
    """
    candidates = []
    for rec in log_records(data):
        ...
        # Si la celda de horas trae etiquetas explícitas (por ejemplo
        # "545 motor / 2234 generador"), cada valor va únicamente a su sistema.
        if system in horas_por_sistema:
            h = horas_por_sistema[system]
        elif d.get("sistema") == system:
            h = d.get("horas")
        else:
            continue
