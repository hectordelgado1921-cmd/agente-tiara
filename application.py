...
def _salidas_barco_2026_record(data):
    """Localiza exclusivamente la fila oficial de SALIDA BARCO para 2026.

    El presupuesto contiene una fila histórica/plural con el mismo concepto.
    Para 2026 la fila singular es la que contiene los valores oficiales
    actuales (incluido TOTAL 2026). Si no existe, se devuelve None para que
    el lector normal conserve su comportamiento anterior.
    """
    for rec in _budget_rows_from_excel_values(data):
...
def _salidas_barco_value(data, year, month=None):
    ...
    """
    # FIX AISLADO 2026: usar la fila oficial singular si existe.
    if year == 2026:
        official_2026 = _salidas_barco_2026_record(data)
        if official_2026:
            if month is not None:
                value = _budget_dict_value(official_2026, year, month)
                if value is not None:
                    return value
            else:
                for key, value in (official_2026.get("data") or {}).items():
                    if norm(key).replace(" ", "") == f"total{year}":
                        parsed = _budget_number(value)
