...
if "pending_voice_type" not in st.session_state:
    st.session_state.pending_voice_type = "audio/webm"
...
# Preguntas por voz: el navegador graba el audio.
# Primero lo dejamos preparado y mostramos un botón explícito para
# enviarlo al intérprete. Así el usuario controla cuándo se procesa.
st.markdown('<div class="voice-question-label">🎙️ O pregunta hablando</div>', unsafe_allow_html=True)
voice_audio = st.audio_input("Pulsa el micrófono y haz tu pregunta", key="tiara_voice_input")
if voice_audio is not None:
    ...
    if audio_signature != st.session_state.voice_audio_signature:
        ...
        st.session_state.pending_voice_audio = audio_bytes
        st.session_state.pending_voice_type = getattr(voice_audio, "type", None) or "audio/webm"

if st.session_state.pending_voice_audio is not None:
    st.caption("🎧 Pregunta grabada. Pulsa el botón para enviarla al intérprete del Agente Tiara.")
    if st.button("🧠 Enviar al intérprete", use_container_width=True, key="send_voice_to_agent"):
        audio_bytes = st.session_state.pending_voice_audio
        audio_type = st.session_state.pending_voice_type
        ...
        # Consumimos el audio después de intentar procesarlo para que el mismo
        # clip no vuelva a ejecutarse en los reruns siguientes.
        st.session_state.pending_voice_audio = None
        if voice_question:
