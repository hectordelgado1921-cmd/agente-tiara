...
def detect_financial_operation(p):
    ...
    # Operación específica del rubro SALIDAS BARCO: únicamente combustible
    # gastado dividido entre número de salidas. No activa otros cálculos.
...
def identify_rubric(p):
    ...
    aliases = [
        ...
        (("salidas barco", "salida barco"), "SALIDAS BARCO"),
    ]
...
def _salidas_barco_record(data):
    """Localiza exclusivamente la fila RUBROS=SALIDAS BARCO del presupuesto."""
    for rec in _budget_rows_from_excel_values(data):
        if _budget_name_matches(_budget_name(rec), "SALIDAS BARCO"):
            return rec
    return None


def _salidas_barco_value(data, year, month=None):
    """Obtiene la cantidad de salidas; no interpreta OBSERVACIONES ni convierte faltantes en cero."""
    ...
    # Los totales oficiales están en la fila inmediatamente siguiente a
    # SALIDAS BARCO en la estructura actual del presupuesto.
    source_row = int(rec.get("source_row") or 0)
    for candidate in _budget_rows_from_excel_values(data):
        if int(candidate.get("source_row") or 0) != source_row + 1:
            continue
    ...
    # Respaldo tolerante si la fila siguiente perdió su número de fila.
    # Solo acepta una fila que contenga el total y que esté inmediatamente
    # después del registro de SALIDAS en el orden del presupuesto.
    rows = _budget_rows_from_excel_values(data)
    try:
        idx = next(i for i, r in enumerate(rows) if r is rec)
    except StopIteration:
        idx = -1
    if idx >= 0 and idx + 1 < len(rows):
        d = rows[idx + 1].get("data") or {}
        for key, value in d.items():
            if norm(key).replace(" ", "") == f"total{year}":
                return _budget_number(value)
...
def _period_specs_for_question(question, thought):
    """Extrae uno o dos períodos explícitos para comparaciones mensuales/anuales."""
    p = norm(question)
    ...
    year_hits = [(m.start(), int(m.group(1))) for m in re.finditer(r"\b(20\d{2})\b", p)]
    ...
    # Si hay dos meses y dos años, emparejar por orden de aparición.
    if len(month_hits) >= 2:
        months = [m[1] for m in month_hits[:2]]
        if len(year_hits) >= 2:
    ...
    # Un solo mes + dos años = mismo mes entre años.
...
def _combustible_budget_value(data, year, month=None):
    """Obtiene el gasto de combustible del presupuesto para el período."""
    if month is not None:
...
def _cost_per_boat_outing(data, year, month=None):
    fuel = _combustible_budget_value(data, year, month)
...
def _answer_combustible_por_salida(data, question, thought):
    specs = _period_specs_for_question(question, thought)
    if not specs:
        return None

    results = []
    for year, month in specs[:2]:
        cost, fuel, outings = _cost_per_boat_outing(data, year, month)
    ...
    if len(results) == 1:
        ...
        return (
            ...
            f"**Salidas de barco:** {outings:g}" if outings is not None else
            "### Análisis financiero — Combustible por salida\n"
            f"**Período:** {label(year, month)}\n"
            f"**Gasto de combustible:** {money(fuel) if fuel is not None else 'NO DETERMINADO'}\n"
            "**Salidas de barco:** NO DETERMINADO"
        ) + (
            f"\n**Combustible promedio por salida:** {money(cost)}"
            if cost is not None else
            "\n**Combustible promedio por salida:** NO DETERMINADO"
    ...
    lines = [
        ...
        f"**Salidas de barco:** {a[4]:g}" if a[4] is not None else "**Salidas de barco:** NO DETERMINADO",
        ...
        f"**Salidas de barco:** {b[4]:g}" if b[4] is not None else "**Salidas de barco:** NO DETERMINADO",
        f"**Combustible promedio por salida:** {money(b[2]) if b[2] is not None else 'NO DETERMINADO'}",
    ]

    if a[2] is None or b[2] is None:
        lines += ["", "**Diferencia USD por salida:** NO DETERMINADO",
                  "**Variación porcentual:** NO DETERMINADA"]
    else:
        diff = b[2] - a[2]
...
def _financial_analysis(data, thought):
    ...
    # SALIDAS BARCO es un rubro de cantidad. Su único cruce financiero
    # aprobado aquí es combustible ÷ salidas, expresado en USD por salida.
    if rubric == "SALIDAS BARCO" and operation == "costo_combustible_por_salida":
...
def run_agent(data, question, context=None):
    ...
    # Consulta directa de cantidad de SALIDAS BARCO, sin interpretar OBSERVACIONES.
    if thought.get("rubric") == "SALIDAS BARCO":
        specs = _period_specs_for_question(question, thought)
        if len(specs) >= 2:
            lines = ["### Salidas de barco"]
            for year, month in specs[:2]:
                value = _salidas_barco_value(data, year, month)
                lines.append(
                    f"**{_financial_period_label(year, month)}:** "
                    f"{value:g}" if value is not None else
                    f"**{_financial_period_label(year, month)}:** NO DETERMINADO"
                )
            return "\n".join(lines) + "\n**Fuente:** PRESUPUESTO TIARA 2026"
        year, month = specs[0]
        value = _salidas_barco_value(data, year, month)
        return (
            ...
            f"**Salidas:** {value:g}" if value is not None else
            "### Salidas de barco\n"
            f"**Período:** {_financial_period_label(year, month)}\n"
            "**Salidas:** NO DETERMINADO"
        ) + "\n**Fuente:** PRESUPUESTO TIARA 2026"
