...
def _salidas_barco_record(data):
    """Selecciona la fila oficial de SALIDA BARCO sin mezclar duplicados.

    En PRESUPUESTO TIARA 2026 existen dos filas con nombres muy parecidos:
    la fila oficial actual usa ``SALIDA BARCO`` (singular), mientras que una
    fila histórica usa ``SALIDAS BARCO`` (plural). La fila singular es la
    autoridad para esta métrica y contiene los totales oficiales 2026/2025.
    """
    records = _budget_rows_from_excel_values(data)

    # 1) Fuente oficial: coincidencia exacta con el rubro singular.
    for rec in records:
        if norm(_budget_name(rec)) == "salida barco":
            return rec

    # 2) Compatibilidad con una futura base que solo conserve el plural.
    for rec in records:
...
def _salidas_barco_value(data, year, month=None):
    """Lee SALIDA BARCO directamente de su fila oficial.

    No utiliza la búsqueda genérica de rubros, porque esa búsqueda puede
    seleccionar la fila histórica/plural. Para un año se usa exclusivamente
    el TOTAL oficial de la fila seleccionada; para un mes se lee el valor del
    mes de esa misma fila. Nunca se utilizan OBSERVACIONES ni se reconstruye
    el total anual sumando otras filas.
    """
    ...
    # El Excel guarda los totales oficiales en la propia fila SALIDA BARCO.
    for key, value in (rec.get("data") or {}).items():
        if norm(key).replace(" ", "") == f"total{year}":
            return _budget_number(value)

    # Si el total oficial no existe, no sustituirlo por una fila duplicada.
    return None
