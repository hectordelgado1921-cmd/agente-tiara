...
APP_VERSION = "2.8.0-stable-improved-lab"
...
def detect_intent(p):
    ...
    if any(x in p for x in ("limpieza", "limpiar", "aseo")): return "limpieza"
    # Una fecha acompañada de una pregunta de evento pertenece a la Bitácora,
    # aunque el usuario no diga literalmente "bitácora".
    if (find_month(p) is not None and re.search(r"\b(que paso|qué pasó|que sucedio|qué sucedió|que ocurrio|qué ocurrió|que hicieron|qué hicieron)\b", p)):
        return "bitacora"
...
def find_day(p):
    # Solo interpreta un día cuando aparece junto a un mes, evitando confundir
    # horómetros, cantidades o años con una fecha de bitácora.
    if find_month(p) is None:
        return None
    m = re.search(r"\b(?:el|dia|día)?\s*(3[01]|[12]\d|0?[1-9])\s*(?:de)?\s*(?:enero|febrero|marzo|abril|mayo|junio|julio|agosto|septiembre|setiembre|octubre|noviembre|diciembre)\b", p)
    if not m:
        return None
...
def budget_value_for_month(data, rubric, year, month):
    """Obtiene el valor del rubro para un mes sin quedarse con una fila incorrecta.

    Si existen varias filas que representan el mismo rubro, primero se busca
    una fila que realmente contenga el mes solicitado. Un vacío, "?" o valor
    no numérico se mantiene como ausencia de dato (NO DETERMINADO).
    """
    rec, value = _find_budget_record_for_month(data, rubric, year, month)
    if rec is not None:
        return value

    # Compatibilidad con estructuras antiguas cuando no fue posible encontrar
    # una fila mensual mediante la estructura normalizada.
...
def budget_total(data, rubric, year):
    ...
    for key, value in d.items():
        if norm(key).replace(" ", "") == f"total{year}":
            ...
            # Un total 0 sin ningún importe mensual real es un vacío de fuente,
            # no un gasto de cero. Conservamos la regla NO DETERMINADO.
            if total_value == 0:
                vals = rec.get("values", [])
                month_cols = (range(3, 27, 2) if year == 2026 else
                              range(4, 28, 2) if year == 2025 else
                              range(3, 27, 2) if year == 2024 else ())
                if not any(_budget_number(vals[c]) is not None for c in month_cols if c < len(vals)):
                    return None
            return total_value
...
def answer_log(data, thought):
    ...
    target_month = thought.get("month")
    target_day = thought.get("day")
    target_year = thought.get("year") or (2026 if target_month is not None else None)
    ...
    for rec in rows:
        ...
        if target_month is not None:
            if rd is None or rd.month != target_month or (target_year is not None and rd.year != target_year):
                continue
            if target_day is not None and rd.day != target_day:
                continue
    ...
    # Consulta por fecha: una vez filtrado el día/mes/año, la fecha ya es la
    # evidencia principal. No exigimos que la palabra "septiembre" aparezca
    # dentro del texto de la observación.
    if target_month is not None:
        candidates.sort(key=lambda r: (_log_date(r) or date.min, int(r.get("source_row") or 0)), reverse=True)
        if not candidates:
            if target_day is not None:
                return f"No encontré entradas de bitácora para el {target_day:02d}-{target_month:02d}-{target_year or 2026}."
            return f"No encontré entradas de bitácora para {MONTH_NAMES[target_month].capitalize()} {target_year or 2026}."
        title_date = (f"{target_day:02d}-{target_month:02d}-{target_year}" if target_day is not None else f"{MONTH_NAMES[target_month].capitalize()} {target_year}")
        lines = [f"### Bitácora — {title_date}"]
        for rec in candidates[:15]:
            lines.append(f"- {_format_log_record(rec)}")
        return "\n".join(lines)
...
def answer_permit_expiry(data, question, context=None):
    """Determina vencimientos desde las celdas del Excel y soporta secuencia.

    El primer pedido devuelve el vencimiento más próximo. En una continuación
    como "¿y el siguiente?" se avanza sobre la misma lista ordenada, sin
    ...
    ordered = future if future else sorted(structured, key=lambda x: x[0])
    index = int(context.get("permit_index", 0) or 0)
    if index < 0:
        index = 0
    if index >= len(ordered):
        return "### Vencimientos — Permisos & Seguros\nNo hay otro vencimiento registrado después de los anteriores."

    dt, rec = ordered[index]
    lines = [
        "### Vencimiento — Permisos & Seguros",
        f"**Documento:** {rec['documento']}",
        f"**Fecha de vencimiento:** {fmt_date(dt)}",
        f"**Posición:** {index + 1} de {len(ordered)}",
        f"**Celda del Excel:** {rec.get('celda_vencimiento') or 'no determinada'}",
    ]
    if index == 0:
        lines.append("**Estado:** es el vencimiento más próximo registrado.")
    else:
        lines.append("**Estado:** siguiente vencimiento en el orden cronológico registrado.")
    return "\n".join(lines)
...
# ============================================================
# CONTEXTO CONVERSACIONAL — CANDIDATA
# Solo interpreta referencias a la conversación anterior. No reemplaza
# detect_intent()/think() ni crea una segunda ruta de respuestas.
# ============================================================

def _conversation_continuation(p, context):
    """Determina si la pregunta actual depende claramente de la anterior."""
    context = context or {}
    if not context.get("intent") and not context.get("rubric") and not context.get("system"):
        return False
    p = norm(p).strip("¿¡!?.,;:").strip()
    if not p:
        return False

    exact = {
        "y", "el mismo", "la misma", "ese mes", "ese mismo mes",
        "ese año", "ese mismo año", "mismo mes", "mismo año",
        "el anterior", "la anterior", "el siguiente", "la siguiente",
        "y el anterior", "y la anterior", "y el siguiente", "y la siguiente",
        "cuanto", "cuánto", "diferencia", "cual fue la diferencia",
        "cuál fue la diferencia", "que toca", "qué toca",
        "que toca hacerle", "qué toca hacerle", "que le toca", "qué le toca",
        "cuantas horas faltan", "cuántas horas faltan", "que le hicieron",
        "qué le hicieron", "que hicieron", "qué hicieron", "cuando fue eso",
        "cuándo fue eso", "cuando fue", "cuándo fue",
    }
    if p in exact:
        return True
    ...
    relative = (
        ...
        "ese año", "ese mismo año", "mismo mes", "mismo año",
        "comparado con", "comparada con", "contra el anterior",
        "contra la anterior", "respecto al anterior", "respecto del anterior",
    )
    if any(x in p for x in relative):
        return True

    # "y julio", "y abril de 2025": continúa la conversación si no introduce
    # un objeto/ruta explícitamente diferente.
    if p.startswith("y "):
        q_without_y = p[2:].strip()
        explicit_topics = (
            "inventario", "existencias", "stock", "combustible", "gasolina",
            "diesel", "diésel", "bitacora", "bitácora", "permiso", "permisos",
            "seguro", "seguros", "documento", "documentos", "foto", "fotografía",
            "factura", "facturas", "limpieza", "mantenimiento", "motor", "motores",
            "generador", "pata", "patas", "horometro", "horómetro",
        )
        if not any(x in q_without_y for x in explicit_topics):
            if find_month(q_without_y) is not None or find_year(q_without_y) is not None:
                return True
            if len(q_without_y.split()) <= 6:
                return True

    return False


def _conversation_apply(thought, context):
    """Completa únicamente campos ausentes con contexto conversacional."""
    p = thought.get("normalized", "")
    continuation = _conversation_continuation(p, context)
    if not continuation:
        return thought, False

    # Si la pregunta trae un objeto/ruta explícitos, no heredamos la ruta previa.
    explicit_intent = thought.get("intent") not in (None, "general")
    if not explicit_intent:
        thought["intent"] = context.get("intent", thought.get("intent"))

    for key in ("system", "month", "year"):
        current = thought.get(key)
        if current in (None, "General"):
            previous = context.get(key)
            if previous is not None:
                thought[key] = previous

    # El rubro no forma parte de think() pero sí de las preguntas financieras.
    if not identify_rubric(p) and context.get("rubric"):
        thought["conversation_rubric"] = context.get("rubric")

    # "año pasado/anterior" desplaza el año de la conversación, sin alterar
    # otros campos. El año de referencia del agente es 2026.
    if any(x in p for x in ("año pasado", "ano pasado", "año anterior", "ano anterior")):
        base_year = context.get("year") or 2026
        thought["year"] = base_year - 1

    # Seguimientos inequívocos: si el cerebro estable detecta una ruta genérica
    # (por ejemplo "horómetros"), la conversación anterior tiene prioridad
    # para expresiones que por sí solas no identifican el tema.
    stripped = p.strip("¿?¡! .,:;").strip()
    previous_intent = context.get("intent")
    if stripped in {"cuantas horas faltan", "cuántas horas faltan", "que toca", "qué toca"}:
        if previous_intent in {"ultimo_mantenimiento", "mantenimiento"}:
            thought["intent"] = "mantenimiento"

    # Secuencia documental: "¿y el siguiente?" / "¿y el anterior?" conserva
    # la ruta de permisos y desplaza una posición en la lista cronológica.
    if stripped in {"el siguiente", "la siguiente", "y el siguiente", "y la siguiente", "siguiente"}:
        if previous_intent == "documentos" or context.get("permit_index") is not None:
            thought["intent"] = "documentos"
            thought["permit_cursor_delta"] = 1
    elif stripped in {"el anterior", "la anterior", "y el anterior", "y la anterior", "anterior"}:
        if previous_intent == "documentos" or context.get("permit_index") is not None:
            thought["intent"] = "documentos"
            thought["permit_cursor_delta"] = -1

    # Continuación temporal de bitácora: "¿y el día siguiente?" / "¿y el
    # día anterior?" desplaza la fecha conservando mes/año cuando corresponde.
    if any(x in stripped for x in ("dia siguiente", "día siguiente", "dia anterior", "día anterior")) and previous_intent == "bitacora":
        base_day = context.get("day")
        base_month = context.get("month")
        base_year = context.get("year") or 2026
        if base_day and base_month:
            try:
                base_date = date(base_year, base_month, base_day)
                delta = 1 if "siguiente" in stripped else -1
                shifted = base_date + __import__("datetime").timedelta(days=delta)
                thought["intent"] = "bitacora"
                thought["day"] = shifted.day
                thought["month"] = shifted.month
                thought["year"] = shifted.year
            except Exception:
                pass

    # "mes pasado/anterior" solo desplaza el mes cuando no hay uno explícito.
    if thought.get("month") is None and any(x in p for x in ("mes pasado", "mes anterior")):
        base_month = context.get("month")
        base_year = context.get("year") or 2026
        if base_month:
            if base_month == 1:
                ...
                thought["month"] = base_month - 1
                thought["year"] = base_year

    return thought, True


def _conversation_save(thought, context):
    """Guarda solo el estado semántico mínimo para el siguiente turno."""
    if context is None:
        return
    rubric = identify_rubric(thought.get("normalized", ""))
    if not rubric:
        rubric = thought.get("conversation_rubric")
    previous_comparison = {
        "comparison_year": context.get("comparison_year"),
        "comparison_month": context.get("comparison_month"),
        "comparison_rubric": context.get("comparison_rubric"),
    }
    previous_permit_index = context.get("permit_index")
    cursor_delta = int(thought.get("permit_cursor_delta", 0) or 0)
    context.clear()
    effective_year = thought.get("year")
    if effective_year is None and thought.get("month") is not None:
        effective_year = 2026
    context.update({
    ...
    if thought.get("intent") == "documentos":
        if cursor_delta:
            base = int(previous_permit_index if previous_permit_index is not None else 0)
            context["permit_index"] = max(0, base + cursor_delta)
        elif previous_permit_index is not None and not _conversation_continuation(thought.get("normalized", ""), {}):
            context["permit_index"] = 0
        elif previous_permit_index is not None:
            context["permit_index"] = previous_permit_index
        else:
            context["permit_index"] = 0
...
def _run_agent_with_context(data, question, context=None):
    thought = think(question)

    # Contexto conversacional INVISIBLE: primero se aplica y DESPUÉS se
    # selecciona la ruta. Esto permite que "¿y el año pasado?", "¿cuándo
    # fue?" o "¿cuánto?" sean realmente continuaciones y no preguntas
    # independientes.
    context = context if context is not None else {}
    thought, continuation = _conversation_apply(thought, context)
    p = thought["normalized"]

    # Referencias temporales que _conversation_apply no necesita inferir desde
    # palabras concretas de mes.
    if continuation:
        if thought.get("month") is None and re.search(
    ...
    # "¿Cuál fue la diferencia?" necesita conservar los dos puntos de
    # comparación. Se resuelve aquí, antes de las rutas productivas, sin crear
    # un segundo cerebro semántico.
    if continuation and p.strip("¿?¡! .,:;").strip() in {
        "diferencia", "cual fue la diferencia", "cuál fue la diferencia",
        "cuanto cambio", "cuánto cambió", "cambio", "variacion", "variación"
    }:
        if context.get("comparison_year") is not None:
            thought["comparison_year"] = context.get("comparison_year")
            thought["comparison_month"] = context.get("comparison_month")
            thought["comparison_rubric"] = context.get("comparison_rubric")
            thought["intent"] = "comparacion_conversacional"

    intent = thought["intent"]

    if intent == "comparacion_conversacional":
        rubric = thought.get("comparison_rubric") or context.get("rubric")
        y_a = thought.get("comparison_year")
        y_b = thought.get("year")
        m_a = thought.get("comparison_month")
        m_b = thought.get("month")
        if rubric and y_a and y_b:
            if m_a:
                va = budget_value_for_month(data, rubric, y_a, m_a)
                vb = budget_value_for_month(data, rubric, y_b, m_b or m_a)
                period = MONTH_NAMES.get(m_a, str(m_a))
            else:
                va = budget_total(data, rubric, y_a)
                vb = budget_total(data, rubric, y_b)
                period = "anual"
            if va is None or vb is None:
                return (
                    f"### Diferencia\n**Rubro:** {rubric}\n"
                    f"**Período:** {period}\n"
                    "**Resultado:** NO DETERMINADO porque falta uno de los dos datos necesarios."
                )
            diff = vb - va
            sign = "aumento" if diff > 0 else ("disminución" if diff < 0 else "sin diferencia")
            return (
                f"### Diferencia\n**Rubro:** {rubric}\n"
                f"**Comparación:** {y_a} → {y_b}\n"
                f"**Diferencia:** {money(abs(diff))} ({sign})\n"
                f"**{y_a}:** {money(va)}\n**{y_b}:** {money(vb)}\n"
                "**Fuente:** PRESUPUESTO TIARA 2026"
            )


    # Las rutas estables siguen siendo las mismas. Para una continuación
    # financiera, el rubro heredado se pasa de forma explícita sin modificar
    # identify_rubric() globalmente.
    if thought.get("conversation_rubric"):
        thought["rubric"] = thought["conversation_rubric"]

    # El cursor documental se calcula para ESTA respuesta. No mutamos el
    # contexto persistente hasta que la respuesta haya terminado.
    answer_context = context
    if thought.get("permit_cursor_delta") is not None:
        answer_context = dict(context)
        base_index = int(context.get("permit_index", 0) or 0)
        answer_context["permit_index"] = max(0, base_index + int(thought.get("permit_cursor_delta") or 0))
    ...
    if intent == "documentos":
        if any(x in p for x in (
            ...
            "mas proximo", "más próximo", "fecha de vencimiento", "siguiente", "anterior"
        )) or thought.get("permit_cursor_delta") is not None:
            return answer_permit_expiry(data, question, answer_context)
...
def run_agent(data, question, context=None):
    if context is None:
        context = {}
    answer = _run_agent_with_context(data, question, context)
    # Guardar el contexto solo después de una respuesta normal. Se conserva
    # además el punto anterior de la conversación para permitir una diferencia
    # entre dos consultas sucesivas.
    try:
        thought = think(question)
        thought, continuation = _conversation_apply(thought, context)
        stripped = question and norm(question).strip("¿?¡! .,:;").strip()
        is_difference_followup = stripped in {
            "diferencia", "cual fue la diferencia", "cuál fue la diferencia",
            "cuanto cambio", "cuánto cambió", "cambio", "variacion", "variación"
        }
        if not is_difference_followup and context.get("year") is not None:
            context["comparison_year"] = context.get("year")
            context["comparison_month"] = context.get("month")
            context["comparison_rubric"] = context.get("rubric")
        _conversation_save(thought, context)
    except Exception:
        pass
    return answer
...
# ============================================================
# RESULTADO ACTUAL — SIN HISTORIAL VISIBLE
# ============================================================
