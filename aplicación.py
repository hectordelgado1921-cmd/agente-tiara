...
def latest_maintenance(kb, q):
    ...
    target_motors = any(x in t for x in ("motor", "motores", "pata motor", "patas motor"))
...
def answer(kb, q):
    ...
    if latest is not None:
        ...
        label = "motores" if any(x in norm(q) for x in ("motor", "motores", "pata motor", "patas motor")) else "mantenimiento"
...
# ============================================================
# CONSULTAS DIRECTAS DE LOS BOTONES
# ============================================================

def button_query(kb, button_name):
    """
    Cada botón tiene una intención fija.
    No usa una búsqueda libre por palabras: llama directamente
    a la función especializada correspondiente.
    """
    if button_name == "estado":
        parts = []

        for sistema in ("Motores Mercury", "Patas Mercury", "Generador Cummins Onan"):
            q = {
                "Motores Mercury": "¿Cuál fue el último mantenimiento de los motores?",
                "Patas Mercury": "¿Cuál fue el último mantenimiento de las patas?",
                "Generador Cummins Onan": "¿Cuál fue el último mantenimiento del generador?",
            }[sistema]

            latest = latest_maintenance(kb, q)
            if latest:
                dt, _rows = latest
                parts.append(f"- **{sistema}:** {dt.strftime('%-d/%-m/%Y')}")
            else:
                parts.append(f"- **{sistema}:** NO DETERMINADO")

        return "### Estado general\n\n" + "\n".join(parts)

    if button_name == "motores":
        latest = latest_maintenance(kb, "¿Cuál fue el último mantenimiento de los motores?")
        if latest:
            dt, _rows = latest
            return (
                "### 🔧 Motores\n\n"
                f"**Último mantenimiento registrado:** {dt.strftime('%-d/%-m/%Y')}"
            )
        return "### 🔧 Motores\n\nNo pude determinar el último mantenimiento de los motores."

    if button_name == "kit300":
        # El botón no inventa datos: consulta el registro de mantenimiento.
        resultados = search(
            kb,
            "kit 300 horas",
            groups=["maintenance_records"],
            limit=5,
        )
        if resultados:
            return "### 🔩 Kit 300 h\n\n" + render(resultados)
        return "### 🔩 Kit 300 h\n\nNo encontré un registro específico de Kit 300 horas."

    if button_name == "patas":
        latest = latest_maintenance(kb, "¿Cuál fue el último mantenimiento de las patas?")
        if latest:
            dt, _rows = latest
            return (
                "### ⚙️ Patas\n\n"
        ...
        return "### ⚙️ Patas\n\nNo pude determinar el último mantenimiento de las patas."

    if button_name == "generador":
        latest = latest_maintenance(kb, "¿Cuál fue el último mantenimiento del generador?")
        if latest:
            dt, _rows = latest
            return (
                "### 🔌 Generador\n\n"
        ...
        return "### 🔌 Generador\n\nNo pude determinar el último mantenimiento del generador."

    return "No se reconoce el botón solicitado."
...
<style>
    ...
    .stButton > button {
        ...
        border: 1px solid #304554 !important;
...
with b1:
    if st.button("🛥️ Estado general", use_container_width=True):
        button_pressed = "estado"

with b2:
    if st.button("🔧 Motores", use_container_width=True):
        button_pressed = "motores"

with b3:
    if st.button("🔩 Kit 300 h", use_container_width=True):
        button_pressed = "kit300"

with b4:
    if st.button("⚙️ Patas", use_container_width=True):
        button_pressed = "patas"

with b5:
    if st.button("🔌 Generador", use_container_width=True):
        button_pressed = "generador"

st.markdown('<div class="section-title">Asistente</div>', unsafe_allow_html=True)

# Solo se mantiene en memoria la interacción actual.
# No existe historial visible ni almacenamiento de conversaciones.
if button_pressed:
    consulta_actual = {
        "estado": "Estado general del barco",
        "motores": "¿Cuál fue el último mantenimiento de los motores?",
        "kit300": "¿Cuál fue el último mantenimiento del kit 300 horas?",
        "patas": "¿Cuál fue el último mantenimiento de las patas?",
        "generador": "¿Cuál fue el último mantenimiento del generador?",
    }[button_pressed]

    respuesta_actual = button_query(kb, button_pressed)

    st.session_state.messages = [
        {"role": "user", "content": consulta_actual},
        {"role": "assistant", "content": respuesta_actual},
    ]

prompt = st.chat_input("Escribe aquí tu consulta sobre el Tiara…")

if prompt:
    # La nueva pregunta reemplaza por completo la interacción anterior.
    st.session_state.messages = [
        {"role": "user", "content": prompt},
        {"role": "assistant", "content": answer(kb, prompt)},
    ]

# Renderizar exclusivamente la interacción actual.
