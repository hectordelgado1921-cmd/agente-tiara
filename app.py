...
def alerts(kb):
    out = []
    for r in kb["inventory_items"]:
        d = r["data"]
        for k, v in d.items():
            if "existencia" in norm(k) and (num(v) or 0) <= 0:
                out.append({
                    "priority": "HIGH",
                    "type": "INVENTARIO",
                    "message": f"Sin existencias: {d.get('PRODUCTO', 'producto sin nombre')}",
                })
    for r in kb["log_records"]:
        d = r["data"]
        for k, v in d.items():
            if norm(k).strip() == "pendiente" and norm(v) not in (
                "", "no", "false", "0", "listo"
            ):
                out.append({
                    "priority": "NORMAL",
                    "type": "BITACORA",
                    "message": f"Pendiente en fila {r['source_row']}",
                })
    return out
...
with right:
    ...
    st.write(f"**Actualizado:** {kb['metadata']['updated_at']}")
