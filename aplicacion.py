...
def _salidas_barco_record(data):
    """Busca SALIDAS BARCO exactamente como cualquier rubro del presupuesto."""
    return _find_budget_record(data, "SALIDAS BARCO")
...
def _salidas_barco_value(data, year, month=None):
    ...
    if month is not None:
        return budget_value_for_month(data, "SALIDAS BARCO", year, month)

    # Primero: mismo mecanismo general del Proyecto de Finanzas.
    value = budget_total(data, "SALIDAS BARCO", year)
    if value is not None:
        return value
...
def answer_inventory(data, thought):
    ...
    if not hits:
        if any(x in p for x in ("inventario", "existencias", "stock")):
            lines = ["### Inventario"]
            for rec in rows[:15]:
                vals = rec.get("values", [])
                if len(vals) >= 3 and vals[1] not in (None, ""):
                    lines.append(f"- **{vals[1]}** — existencias: **{vals[2] if vals[2] not in (None, '') else 'NO DETERMINADO'}**")
            return "\n".join(lines)
...
def _conversation_is_difference(p):
    return norm(p) in {
...
def _conversation_is_previous_year(p):
    p = norm(p)
    return bool(re.search(r"\b(año pasado|ano pasado|el año pasado|el ano pasado)\b", p))


def _conversation_is_next_previous(p):
    p = norm(p)
    if re.search(r"\b(anterior|previo|previa)\b", p):
        return -1
    if re.search(r"\b(siguiente|proximo|próximo)\b", p):
        return 1
    return 0


def _conversation_date_question(p):
    """Reconoce preguntas de bitácora por una fecha aunque no digan 'bitácora'."""
    p = norm(p)
    return bool(
        re.search(r"\b(que paso|qué pasó|que sucedio|qué sucedió|que ocurrió|qué ocurrió)\b", p)
        and (find_month(p) is not None or re.search(r"\b\d{1,2}[/-]\d{1,2}\b", p))
    )


def _conversation_extract_date(p, default_year=2026):
    p = norm(p)
    m = re.search(r"\b(\d{1,2})\s+de\s+([a-záéíóúñ]+)(?:\s+de\s+(20\d{2}))?\b", p)
    if m:
        month = MONTHS.get(m.group(2))
        if month:
            return date(int(m.group(3) or default_year), month, int(m.group(1)))
    m = re.search(r"\b(\d{1,2})[/-](\d{1,2})(?:[/-](20\d{2}))?\b", p)
    if m:
        return date(int(m.group(3) or default_year), int(m.group(2)), int(m.group(1)))
    return None


def _answer_log_exact_date(data, target_date):
    rows = log_records(data)
    matches = [r for r in rows if _log_date(r) == target_date]
    if not matches:
        return f"No encontré registros de bitácora para el {fmt_date(target_date)}."
    matches.sort(key=lambda r: int(r.get("source_row") or 0))
    lines = [f"### Bitácora — {fmt_date(target_date)}"]
    for rec in matches:
        lines.append(f"- {_format_log_record(rec)}")
    return "\n".join(lines)


def _conversation_difference_answer(data, context):
    rubric = context.get("rubric")
    month = context.get("month")
    year_a = context.get("comparison_year")
    year_b = context.get("year")
    if not rubric or year_a is None or year_b is None:
        return None
    if month is not None:
        a = budget_value_for_month(data, rubric, year_a, month)
        b = budget_value_for_month(data, rubric, year_b, month)
        period = MONTH_NAMES.get(month, str(month)).capitalize()
    else:
        a = budget_total(data, rubric, year_a)
        b = budget_total(data, rubric, year_b)
        period = "total anual"
    if a is None or b is None:
        return "No se puede determinar la diferencia porque falta uno de los dos valores necesarios."
    diff = a - b
    direction = "aumento" if diff > 0 else "disminución" if diff < 0 else "sin cambio"
    return (
        f"### Diferencia\n"
        f"**Rubro:** {rubric}\n"
        f"**Período:** {period}\n"
        f"**{year_a}:** {money(a)}\n"
        f"**{year_b}:** {money(b)}\n"
        f"**Diferencia:** {money(abs(diff))} — **{direction}** en {year_a} respecto a {year_b}."
    )


def _conversation_documents_relative(data, context, step):
    records = data.get("permit_records") or []
    structured = []
    for r in records:
        raw = r.get("fecha_vencimiento")
    ...
    if not structured:
        return None
    today = date.today()
    ordered = sorted([x for x in structured if x[0] >= today], key=lambda x: x[0])
    if not ordered:
        ordered = sorted(structured, key=lambda x: x[0])
    current = int(context.get("permit_index", 0) or 0)
    target = current + step
    target = max(0, min(target, len(ordered) - 1))
    context["permit_index"] = target
    dt, rec = ordered[target]
    ordinal = target + 1
    return (
        f"### Vencimiento #{ordinal}\n"
        f"**Documento:** {rec['documento']}\n"
        f"**Fecha de vencimiento:** {fmt_date(dt)}"
    )


def _conversation_update_context(context, thought, question):
    """Guarda solo estado semántico mínimo para el siguiente turno."""
    p = thought.get("normalized", norm(question))
    if _conversation_is_difference(p):
        return
    if _conversation_is_previous_year(p) and context.get("rubric"):
        context["comparison_year"] = context.get("year")
        context["year"] = (context.get("year") or 2026) - 1
        return
    # Solo una consulta sustantiva reemplaza el tema anterior.
    if thought.get("rubric"):
        context["rubric"] = thought.get("rubric")
    if thought.get("system") and thought.get("system") != "General":
        context["system"] = thought.get("system")
    if thought.get("intent") and thought.get("intent") != "general":
        context["intent"] = thought.get("intent")
    if thought.get("month") is not None:
        context["month"] = thought.get("month")
    if thought.get("year") is not None:
        context["year"] = thought.get("year")
    if context.get("year") is not None and context.get("comparison_year") is None:
        context["comparison_year"] = None
...
def run_agent(data, question, context=None):
    ...
    # ========================================================
    # CONTEXTO CONVERSACIONAL — SOLO SEMÁNTICO
    # ========================================================
    # Nunca se guarda la conversación ni la respuesta anterior. Solo los
    # parámetros necesarios para resolver referencias como "el año pasado",
    # "la diferencia", "el siguiente" o "qué pasó el día siguiente".
    ...
    # Heredar tema/sistema/rubro cuando la nueva pregunta es una continuación.
    continuation = (
        _conversation_is_difference(p)
        or _conversation_is_previous_year(p)
        or _conversation_is_next_previous(p) != 0
        or p in {"cuando fue", "cuándo fue", "cuantas horas faltan", "cuántas horas faltan", "que toca", "qué toca"}
        or bool(re.search(r"\bdia siguiente\b|\bdía siguiente\b", p))
    )
    if continuation:
        if thought.get("system") == "General":
            thought["system"] = context.get("system", thought.get("system"))
        if not thought.get("rubric"):
            thought["rubric"] = context.get("rubric")
        if thought.get("year") is None:
            thought["year"] = context.get("year")
        if thought.get("month") is None:
            thought["month"] = context.get("month")

    # "¿Y el año pasado?" conserva el rubro y el mes anterior.
    if _conversation_is_previous_year(p):
        base_year = context.get("year") or thought.get("year") or 2026
        thought["year"] = base_year - 1
        thought["years"] = [thought["year"]]
        context["comparison_year"] = base_year
        context["year"] = base_year - 1

    # Preguntas de diferencia usan exactamente los dos períodos de la consulta
    # anterior, sin volver a interpretar "diferencia" como un rubro nuevo.
    if _conversation_is_difference(p):
        answer = _conversation_difference_answer(data, context)
        if answer is not None:
            return answer

    # "¿Y el siguiente/anterior?" sobre vencimientos usa orden cronológico.
    relative = _conversation_is_next_previous(p)
    if relative and context.get("document_context"):
        answer = _conversation_documents_relative(data, context, relative)
        if answer is not None:
            return answer

    # Fechas de bitácora sin mencionar la palabra bitácora.
    if _conversation_date_question(p):
        target = _conversation_extract_date(p, context.get("year") or 2026)
        if target:
            context["log_date"] = target.isoformat()
            context["system"] = "Bitácora"
            return _answer_log_exact_date(data, target)

    # "¿Y el día siguiente?" conserva la fecha de la bitácora.
    if re.search(r"\bdia siguiente\b|\bdía siguiente\b", p) and context.get("log_date"):
        target = date.fromisoformat(context["log_date"]) + __import__("datetime").timedelta(days=1)
        context["log_date"] = target.isoformat()
        return _answer_log_exact_date(data, target)

    # Mantenimiento: estas formas deben conservar el sistema anterior.
    if thought.get("intent") in {"mantenimiento", "ultimo_mantenimiento"} and thought.get("system") == "General":
        thought["system"] = context.get("system", "General")

    _conversation_update_context(context, thought, question)
    ...
    if uploaded_file is not None:
        ...
        if st.session_state.uploaded_signature != signature:
            ...
            try:
                ...
                st.session_state.query_context = {"month": None, "year": None, "rubric": None, "system": None, "intent": None, "comparison_year": None, "permit_index": 0, "document_context": False, "log_date": None}
                # Un Excel nuevo es una nueva fuente de verdad: no mezclamos
                # conversaciones hechas sobre una base anterior.
...
if "query_context" not in st.session_state:
    st.session_state.query_context = {"month": None, "year": None, "rubric": None, "system": None, "intent": None, "comparison_year": None, "permit_index": 0, "document_context": False, "log_date": None}
# Historial real de la conversación durante la sesión de Streamlit.
# Se conserva separado del contexto semántico para que el cerebro pueda usar
# el contexto estructurado sin tener que reinterpretar todo el texto anterior.
...
if quick:
    # Ejecutar con el contexto semántico acumulado de la conversación.
    # El historial completo se conserva aparte para no contaminar el cerebro
    # determinista con texto libre innecesario.
    question_text = str(quick).strip()
    answer_text = run_agent(
        data, question_text, st.session_state.query_context
    )
    st.session_state.last_question = question_text
    st.session_state.last_answer = answer_text

    # Guardar el turno completo. Esto permite conservar la conversación dentro
    # de la sesión de Streamlit y deja disponible el contexto anterior aunque
    # haya varios turnos consecutivos. No se mezcla con los datos del Excel.
    st.session_state.chat_history.append({
        "timestamp": datetime.now(APP_TIMEZONE).isoformat(),
        "question": question_text,
        "answer": answer_text,
    })
    # Evitar crecimiento ilimitado de memoria en sesiones muy largas.
    if len(st.session_state.chat_history) > 100:
        st.session_state.chat_history = st.session_state.chat_history[-100:]

    # Mantener el contexto estructurado actual para las referencias semánticas.
    # run_agent ya actualiza los campos que corresponden a continuaciones;
    # aquí solo completamos datos explícitos que no hayan quedado registrados.
    new_thought = think(question_text)
    if new_thought.get("month") is not None:
        st.session_state.query_context["month"] = new_thought["month"]
    if new_thought.get("year") is not None and not _conversation_is_previous_year(new_thought.get("normalized", "")):
