...
if _supports_audio_chat:
    ...
    if _chat_value is not None:
        # Compatibilidad: distintas versiones de Streamlit pueden devolver
        # directamente un string o un objeto con .text/.audio.
        if isinstance(_chat_value, str):
            typed_question = _chat_value.strip()
            voice_audio = None
        else:
            typed_question = (getattr(_chat_value, "text", "") or "").strip()
            voice_audio = getattr(_chat_value, "audio", None)

            # Algunas implementaciones pueden entregar el valor como mapping.
            if not typed_question and isinstance(_chat_value, dict):
                typed_question = str(_chat_value.get("text") or "").strip()
